...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = PaysQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Pays();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Pays';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_Pays($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        try{
                            $e->setLocale('fr_CA');
                            if(isset($data['PaysI18n_Name_frCA'])){$e->setName($data['PaysI18n_Name_frCA']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new PaysI18n();
                            $mt->setLocale('fr_CA');
                            
                            if(isset($data['PaysI18n_Name_frCA'])){$mt->setName($data['PaysI18n_Name_frCA']);}
                            $e->addPaysI18n($mt);$e->save();
                        }
                    
                        try{
                            $e->setLocale('en_US');
                            if(isset($data['PaysI18n_Name_enUS'])){$e->setName($data['PaysI18n_Name_enUS']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new PaysI18n();
                            $mt->setLocale('en_US');
                            
                            if(isset($data['PaysI18n_Name_enUS'])){$mt->setName($data['PaysI18n_Name_enUS']);}
                            $e->addPaysI18n($mt);$e->save();
                        }
                    
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Pays'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Pays'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_Pays($data, $request);
                    
                    
                        
                        
                        
                    $e->setLocale('fr_CA');$e->setName($data['PaysI18n_Name_frCA']);
                    $e->setLocale('en_US');$e->setName($data['PaysI18n_Name_enUS']);
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        $e->save();
                        
                        unset($_SESSION['memoire']['onglet']['Pays']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Pays']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Pays']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Pays']['i'] = set_var($_SESSION['memoire']['onglet']['Pays']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Pays'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Pays';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new PaysForm();
                                $e->Pays['request'] = $request;
                                $e->Pays['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new PaysForm();
                $f->Pays['request'] = $request;
                if(!empty($request['ip'])){$f->Pays['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Pays']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Pays']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdPays';
                    ...
                    unset($_SESSION['memoire']['onglet']['Pays']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = PaysQuery::create()->findPk($request['i']);
                
            if($obj->countAccounts()){ $error = handleNotOkResponse(_("Cette entrée ne peut être supprimée car elle est présentement utilisée dans")." 'Compte'. ", '', true,'Pays'); die( $error['onReadyJs'] ); }
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Pays';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Pays",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Pays",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Pays",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Pays",'r')){
            init_list_base($request,$search,'Pays','','','','');
            $e = new PaysForm();
            $e->Pays['request'] = $request;
            if(!empty($request['ip'])){$e->Pays['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderPays']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsPays'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Pays($data, $request=array()){
        $e = new PaysForm();
        $e->Pays['request'] = $request;
        return $e->save_update_Pays($data);
    }
    function save_create_Pays($data, $request=array()){
        $e = new PaysForm();
        $e->Pays['request'] = $request;
        return $e->save_create_Pays($data);
    }
