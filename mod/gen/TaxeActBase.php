...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = TaxeQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Taxe();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Taxe';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_Taxe($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        try{
                            $e->setLocale('fr_CA');
                            if(isset($data['TaxeI18n_Title_frCA'])){$e->setTitle($data['TaxeI18n_Title_frCA']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new TaxeI18n();
                            $mt->setLocale('fr_CA');
                            
                            if(isset($data['TaxeI18n_Title_frCA'])){$mt->setTitle($data['TaxeI18n_Title_frCA']);}
                            $e->addTaxeI18n($mt);$e->save();
                        }
                    
                        try{
                            $e->setLocale('en_US');
                            if(isset($data['TaxeI18n_Title_enUS'])){$e->setTitle($data['TaxeI18n_Title_enUS']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new TaxeI18n();
                            $mt->setLocale('en_US');
                            
                            if(isset($data['TaxeI18n_Title_enUS'])){$mt->setTitle($data['TaxeI18n_Title_enUS']);}
                            $e->addTaxeI18n($mt);$e->save();
                        }
                    ...
                    $e = save_create_Taxe($data, $request);
                    
                    
                        
                        
                        
                    $e->setLocale('fr_CA');$e->setTitle($data['TaxeI18n_Title_frCA']);
                    $e->setLocale('en_US');$e->setTitle($data['TaxeI18n_Title_enUS']);
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        $e->save();
                        
                        unset($_SESSION['memoire']['onglet']['Taxe']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Taxe']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Taxe']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Taxe']['i'] = set_var($_SESSION['memoire']['onglet']['Taxe']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);

                        }
                        
                        
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $data['IdGroupTaxeSup'] = json_encode($e->getIdGroupTaxeSup());
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Taxe';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        ...
                        if(!$data['IdGroupTaxeSup'] && !$request['ip'] or ($request['je'] =='TaxeTable' and !$request['ip']) ){
                            $output['onReadyJs'] .= "$('#formMsTaxe #msTaxeBt').click();".$closeDiag."";
                        }else{
                            $output['onReadyJs'] .= "$('.child_pannel [p=\"Taxe\"]').click();".$closeDiag."";
                        }
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new TaxeForm();
                                $e->Taxe['request'] = $request;
                                $e->Taxe['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                if(!empty($request['ip'])){$f->Taxe['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Taxe']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Taxe']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdTaxe';
                    ...
                    unset($_SESSION['memoire']['onglet']['Taxe']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $obj = TaxeQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Taxe';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Taxe",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Taxe",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Taxe",'r')
            ...
            if(!empty($request['ip'])){
                $relData['IdGroupTaxeSup'] = $request['ip'];
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Taxe",'r')){
            init_list_base($request,$search,'Taxe','','','','');
            $e = new TaxeForm();
            $e->Taxe['request'] = $request;
            if(!empty($request['ip'])){$e->Taxe['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderTaxe']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsTaxe'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Taxe($data, $request=array()){
        $e = new TaxeForm();
        $e->Taxe['request'] = $request;
        return $e->save_update_Taxe($data);
    }
    function save_create_Taxe($data, $request=array()){
        $e = new TaxeForm();
        $e->Taxe['request'] = $request;
        return $e->save_create_Taxe($data);
    }
