...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = StopWordQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new StopWord();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'StopWord';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_StopWord($data, $request);
                    ...
                    $e = save_create_StopWord($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['StopWord']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['StopWord']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['StopWord']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['StopWord']['i'] = set_var($_SESSION['memoire']['onglet']['StopWord']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'StopWord';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdStopword'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new StopWordForm();
                                $e->StopWord['request'] = $request;
                                $e->StopWord['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new StopWordForm();
                $f->StopWord['request'] = $request;
                if(!empty($request['ip'])){$f->StopWord['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['StopWord']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['StopWord']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdStopword';
                    ...
                    unset($_SESSION['memoire']['onglet']['StopWord']['ixmemautoc']);
                }
                ...
                $IdCurrentStopWord = $data['i'];
                
            $q = StopWordQuery::create()->filterByIdStopword($IdCurrentStopWord)->setFormatter(ModelCriteria::FORMAT_ARRAY)->findOne();

            @$output['onReadyJs'] .= "
                $('#formStopWord .divtr.readonly .divtd[in=\"inIdStopword\"] label.readonly').text('".addslashes($q['IdStopword'])."');
                $('#formStopWord #IdStopword').val('".addslashes($q['IdStopword'])."');
            ";
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = StopWordQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'StopWord';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."StopWord",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."StopWord",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."StopWord",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("StopWord",'r')){
            init_list_base($request,$search,'StopWord','','','','');
            $e = new StopWordForm();
            $e->StopWord['request'] = $request;
            if(!empty($request['ip'])){$e->StopWord['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderStopWord']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsStopWord'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_StopWord($data, $request=array()){
        $e = new StopWordForm();
        $e->StopWord['request'] = $request;
        return $e->save_update_StopWord($data);
    }
    function save_create_StopWord($data, $request=array()){
        $e = new StopWordForm();
        $e->StopWord['request'] = $request;
        return $e->save_create_StopWord($data);
    }
