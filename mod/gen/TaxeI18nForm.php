...
###############################
/**
 * Form class on the 'TaxeI18n' table.
 *
 */
class TaxeI18nForm extends TaxeI18n{
public $tableName="TaxeI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('TaxeI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."TaxeI18n/edit/", "id='addTaxeI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addTaxeI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['TaxeI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['TaxeI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'TaxeI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'TaxeI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#TaxeI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#TaxeI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('TaxeI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('TaxeI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('TaxeI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteTaxeI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='TaxeI18nRow".$data->getPrimaryKey()."'
                    data-table='TaxeI18n' 
            ...
            $tr .= input('hidden', 'rowCountTaxeI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'TaxeI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='TaxeI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
        ...
        if(empty($hasParent)){
            $editEvent = "
        $(\"#TaxeI18nListForm [j='deleteTaxeI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msTaxeI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msTaxeI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#TaxeI18nListForm tr[ecf=1] td[j='editTaxeI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."TaxeI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."TaxeI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#TaxeI18nListForm [j='deleteTaxeI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msTaxeI18nBt\').length>0){ $(\'#msTaxeI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'TaxeI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntTaxeI18nListForm #addTaxeI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'TaxeI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#TaxeI18nListForm tr[ecf=1] td[j='editTaxeI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'TaxeI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('TaxeI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#TaxeI18nListForm [j='deleteTaxeI18n']\").unbind('click');
        $('#TaxeI18nListForm #addTaxeI18n').unbind('click');
        $(\"#TaxeI18nListForm tr[ecf=1] td[j='editTaxeI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#TaxeI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('TaxeI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('TaxeI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_TaxeI18n($data){
    ...
    public function save_update_TaxeI18n($data){

        
        $e = TaxeI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of TaxeI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['TaxeI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['TaxeI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addTaxeI18n_child').bind('click.addTaxeI18n', function (){
                    $.post('"._SITE_URL."mod/act/TaxeI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addTaxeI18n_child').bind('click.addTaxeI18n', function (){
                document.location= '"._SITE_URL."TaxeI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'TaxeI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'TaxeI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formTaxeI18n #saveTaxeI18n').removeAttr('disabled');
                    $('#formTaxeI18n #saveTaxeI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formTaxeI18n #saveTaxeI18n').css('cursor', 'default');
                    if($('#formTaxeI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formTaxeI18n #saveTaxeI18n').bind('click.saveTaxeI18n', function (data){
                    $('#formTaxeI18n #saveTaxeI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formTaxeI18n #saveTaxeI18n').css('cursor', 'progress');
                    $('#formTaxeI18n #saveTaxeI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formTaxeI18n .tinymce').each(function(index) {
                        eval(' $(\"#formTaxeI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formTaxeI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formTaxeI18n\").find(\"[s='d']\").serializeArray();
                        $('#formTaxeI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formTaxeI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/TaxeI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formTaxeI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formTaxeI18n #formChangedTaxeI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formTaxeI18n #saveTaxeI18n').unbind('click.saveTaxeI18n');
                $('#formTaxeI18n #saveTaxeI18n').remove();";
        }
        
        if($dataObj == null){
            $this->TaxeI18n['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new TaxeI18n();
            $this->TaxeI18n['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->TaxeI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdTaxe()){
            $this->printLink =
                ...
                ,'TaxeI18nControls', "class='custom-controls'");
        ...
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            ...
            $this->add_button = 
                ...
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addTaxeI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'TaxeI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formTaxeI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['TaxeI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['TaxeI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['TaxeI18n']['Start']['html']
                
                
                
                .@$this->fields['TaxeI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntTaxeI18n", "class='divStdform' CntTabs=1 ")
        , "id='formTaxeI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdTaxe()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelTaxeI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntTaxeI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['TaxeI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['TaxeI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['TaxeI18n']['ixmemautocapp']);
        }
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntTaxeI18n');},500); 
    ".$toggleForm."
    bind_form('TaxeI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['TaxeI18n'] as $field=>$ar){
                $this->fields['TaxeI18n'][$field]['html'] = $this->fieldsRo['TaxeI18n'][$field]['html'];
            }
    ...
    */
    ...
    $this->ajaxPageAct = 'TaxeI18nAct.php';
