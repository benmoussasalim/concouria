...
###############################
/**
 * Form class on the 'MessageI18n' table.
 *
 */
class MessageI18nForm extends MessageI18n{
public $tableName="MessageI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('MessageI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."MessageI18n/edit/", "id='addMessageI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addMessageI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['MessageI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['MessageI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'MessageI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'MessageI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#MessageI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#MessageI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('MessageI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('MessageI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('MessageI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteMessageI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='MessageI18nRow".$data->getPrimaryKey()."'
                    data-table='MessageI18n' 
            ...
            $tr .= input('hidden', 'rowCountMessageI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'MessageI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='MessageI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
        ...
        if(empty($hasParent)){
            $editEvent = "
        $(\"#MessageI18nListForm [j='deleteMessageI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msMessageI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msMessageI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#MessageI18nListForm tr[ecf=1] td[j='editMessageI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."MessageI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."MessageI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#MessageI18nListForm [j='deleteMessageI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msMessageI18nBt\').length>0){ $(\'#msMessageI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'MessageI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntMessageI18nListForm #addMessageI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'MessageI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#MessageI18nListForm tr[ecf=1] td[j='editMessageI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'MessageI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('MessageI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#MessageI18nListForm [j='deleteMessageI18n']\").unbind('click');
        $('#MessageI18nListForm #addMessageI18n').unbind('click');
        $(\"#MessageI18nListForm tr[ecf=1] td[j='editMessageI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#MessageI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('MessageI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('MessageI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_MessageI18n($data){
    ...
    public function save_update_MessageI18n($data){

        
        $e = MessageI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of MessageI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['MessageI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['MessageI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addMessageI18n_child').bind('click.addMessageI18n', function (){
                    $.post('"._SITE_URL."mod/act/MessageI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addMessageI18n_child').bind('click.addMessageI18n', function (){
                document.location= '"._SITE_URL."MessageI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'MessageI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'MessageI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formMessageI18n #saveMessageI18n').removeAttr('disabled');
                    $('#formMessageI18n #saveMessageI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formMessageI18n #saveMessageI18n').css('cursor', 'default');
                    if($('#formMessageI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formMessageI18n #saveMessageI18n').bind('click.saveMessageI18n', function (data){
                    $('#formMessageI18n #saveMessageI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formMessageI18n #saveMessageI18n').css('cursor', 'progress');
                    $('#formMessageI18n #saveMessageI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formMessageI18n .tinymce').each(function(index) {
                        eval(' $(\"#formMessageI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formMessageI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formMessageI18n\").find(\"[s='d']\").serializeArray();
                        $('#formMessageI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formMessageI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/MessageI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formMessageI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formMessageI18n #formChangedMessageI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formMessageI18n #saveMessageI18n').unbind('click.saveMessageI18n');
                $('#formMessageI18n #saveMessageI18n').remove();";
        }
        
        if($dataObj == null){
            $this->MessageI18n['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new MessageI18n();
            $this->MessageI18n['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->MessageI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdMessage()){
            $this->printLink =
                ...
                ,'MessageI18nControls', "class='custom-controls'");
        ...
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            ...
            $this->add_button = 
                ...
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addMessageI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'MessageI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formMessageI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['MessageI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['MessageI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['MessageI18n']['Start']['html']
                
                
                
                .@$this->fields['MessageI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntMessageI18n", "class='divStdform' CntTabs=1 ")
        , "id='formMessageI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdMessage()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelMessageI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntMessageI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['MessageI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['MessageI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['MessageI18n']['ixmemautocapp']);
        }
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntMessageI18n');},500); 
    ".$toggleForm."
    bind_form('MessageI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['MessageI18n'] as $field=>$ar){
                $this->fields['MessageI18n'][$field]['html'] = $this->fieldsRo['MessageI18n'][$field]['html'];
            }
    ...
    */
    ...
    $this->ajaxPageAct = 'MessageI18nAct.php';
