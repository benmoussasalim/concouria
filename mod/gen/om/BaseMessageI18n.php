...
abstract class BaseMessageI18n extends BaseObject implements Persistent
{
    ...
    protected function loadDateCreation(PropelPDO $con = null)
    {
        ...
        $c->addSelectColumn(MessageI18nPeer::DATE_CREATION);
        try {
            $stmt = MessageI18nPeer::doSelectStmt($c, $con);
    ...
    {
        $c = $this->buildPkeyCriteria();
        $c->addSelectColumn(MessageI18nPeer::DATE_MODIFICATION);
        try {
            $stmt = MessageI18nPeer::doSelectStmt($c, $con);
            $row = $stmt->fetch(PDO::FETCH_NUM);
            $stmt->closeCursor();
    ...
    public function reload($deep = false, PropelPDO $con = null){
        ...
        if ($con === null) { $con = Propel::getConnection(MessageI18nPeer::DATABASE_NAME, Propel::CONNECTION_READ);}
...
// TableStampBehavior behavior
        ...
        if(!$_SESSION['CurrentRights']){$_SESSION['CurrentRights']='MessageI18n';}
        $needeRight='d';
        ...
        mem_clean('MessageI18n');

        if ($this->isDeleted()) { throw new PropelException("This object has already been deleted.");}
        if ($con === null) { $con = Propel::getConnection(MessageI18nPeer::DATABASE_NAME, Propel::CONNECTION_WRITE);}
        $con->beginTransaction();
        try {
            $deleteQuery = MessageI18nQuery::create()->filterByPrimaryKey($this->getPrimaryKey());
...
// TableStampBehavior behavior
        ...
        if(!$_SESSION[_AUTH_VAR]->hasRights($_SESSION['CurrentRights'],$needeRight,$this)){
            ...
            if ($this->aMessage !== null) {
                if (!$this->aMessage->validate($columns)) {$failureMap = array_merge($failureMap, $this->aMessage->getValidationFailures()); }
            }

            if (($retval = MessageI18nPeer::doValidate($this, $columns)) !== true){$failureMap = array_merge($failureMap, $retval);}
