...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = BlockI18nVersionQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new BlockI18nVersion();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'BlockI18nVersion';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_BlockI18nVersion($data, $request);
                    ...
                    $e = save_create_BlockI18nVersion($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['BlockI18nVersion']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['BlockI18nVersion']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['BlockI18nVersion']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['BlockI18nVersion']['i'] = set_var($_SESSION['memoire']['onglet']['BlockI18nVersion']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'BlockI18nVersion';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdBlockI18nVersion'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new BlockI18nVersionForm();
                                $e->BlockI18nVersion['request'] = $request;
                                $e->BlockI18nVersion['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new BlockI18nVersionForm();
                $f->BlockI18nVersion['request'] = $request;
                if(!empty($request['ip'])){$f->BlockI18nVersion['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['BlockI18nVersion']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['BlockI18nVersion']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdBlockI18nVersion';
                    ...
                    unset($_SESSION['memoire']['onglet']['BlockI18nVersion']['ixmemautoc']);
                }
                ...
                $IdCurrentBlockI18nVersion = $data['i'];
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = BlockI18nVersionQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'BlockI18nVersion';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."BlockI18nVersion",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."BlockI18nVersion",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."BlockI18nVersion",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("BlockI18nVersion",'r')){
            init_list_base($request,$search,'BlockI18nVersion','','','','');
            $e = new BlockI18nVersionForm();
            $e->BlockI18nVersion['request'] = $request;
            if(!empty($request['ip'])){$e->BlockI18nVersion['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderBlockI18nVersion']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsBlockI18nVersion'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_BlockI18nVersion($data, $request=array()){
        $e = new BlockI18nVersionForm();
        $e->BlockI18nVersion['request'] = $request;
        return $e->save_update_BlockI18nVersion($data);
    }
    function save_create_BlockI18nVersion($data, $request=array()){
        $e = new BlockI18nVersionForm();
        $e->BlockI18nVersion['request'] = $request;
        return $e->save_create_BlockI18nVersion($data);
    }
