...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = MonthWinnerQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new MonthWinner();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'MonthWinner';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_MonthWinner($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        try{
                            $e->setLocale('fr_CA');
                            if(isset($data['MonthWinnerI18n_Name_frCA'])){$e->setName($data['MonthWinnerI18n_Name_frCA']);}
                            if(isset($data['MonthWinnerI18n_Text_frCA'])){$e->setText($data['MonthWinnerI18n_Text_frCA']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new MonthWinnerI18n();
                            $mt->setLocale('fr_CA');
                            
                            if(isset($data['MonthWinnerI18n_Name_frCA'])){$mt->setName($data['MonthWinnerI18n_Name_frCA']);}
                            if(isset($data['MonthWinnerI18n_Text_frCA'])){$mt->setText($data['MonthWinnerI18n_Text_frCA']);}
                            $e->addMonthWinnerI18n($mt);$e->save();
                        }
                    
                        try{
                            $e->setLocale('en_US');
                            if(isset($data['MonthWinnerI18n_Name_enUS'])){$e->setName($data['MonthWinnerI18n_Name_enUS']);}
                            if(isset($data['MonthWinnerI18n_Text_enUS'])){$e->setText($data['MonthWinnerI18n_Text_enUS']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new MonthWinnerI18n();
                            $mt->setLocale('en_US');
                            
                            if(isset($data['MonthWinnerI18n_Name_enUS'])){$mt->setName($data['MonthWinnerI18n_Name_enUS']);}
                            if(isset($data['MonthWinnerI18n_Text_enUS'])){$mt->setText($data['MonthWinnerI18n_Text_enUS']);}
                            $e->addMonthWinnerI18n($mt);$e->save();
                        }
                    
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Mois'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Mois'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_MonthWinner($data, $request);
                    
                    
                        
                        
                        
                    $e->setLocale('fr_CA');$e->setName($data['MonthWinnerI18n_Name_frCA']);
                    $e->setLocale('fr_CA');$e->setText($data['MonthWinnerI18n_Text_frCA']);
                    $e->setLocale('en_US');$e->setName($data['MonthWinnerI18n_Name_enUS']);
                    $e->setLocale('en_US');$e->setText($data['MonthWinnerI18n_Text_enUS']);
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        $e->save();
                        
                        unset($_SESSION['memoire']['onglet']['MonthWinner']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['MonthWinner']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['MonthWinner']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['MonthWinner']['i'] = set_var($_SESSION['memoire']['onglet']['MonthWinner']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);

                        }
                        
                        
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $data['IdWinner'] = json_encode($e->getIdWinner());
                        
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Mois'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'MonthWinner';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        ...
                        if(!$data['IdWinner'] && !$request['ip'] or ($request['je'] =='MonthWinnerTable' and !$request['ip']) ){
                            $output['onReadyJs'] .= "$('#formMsMonthWinner #msMonthWinnerBt').click();".$closeDiag."";
                        }else{
                            $output['onReadyJs'] .= "$('.child_pannel [p=\"MonthWinner\"]').click();".$closeDiag."";
                        }
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new MonthWinnerForm();
                                $e->MonthWinner['request'] = $request;
                                $e->MonthWinner['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new MonthWinnerForm();
                $f->MonthWinner['request'] = $request;
                if(!empty($request['ip'])){$f->MonthWinner['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['MonthWinner']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['MonthWinner']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdMonthWinner';
                    ...
                    unset($_SESSION['memoire']['onglet']['MonthWinner']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $obj = MonthWinnerQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'MonthWinner';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."MonthWinner",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."MonthWinner",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."MonthWinner",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("MonthWinner",'r')){
            init_list_base($request,$search,'MonthWinner','','','','');
            $e = new MonthWinnerForm();
            $e->MonthWinner['request'] = $request;
            if(!empty($request['ip'])){$e->MonthWinner['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderMonthWinner']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsMonthWinner'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_MonthWinner($data, $request=array()){
        $e = new MonthWinnerForm();
        $e->MonthWinner['request'] = $request;
        return $e->save_update_MonthWinner($data);
    }
    function save_create_MonthWinner($data, $request=array()){
        $e = new MonthWinnerForm();
        $e->MonthWinner['request'] = $request;
        return $e->save_create_MonthWinner($data);
    }
