...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = BlockQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Block();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Block';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    beforeSave($e, $data, false, $output, $extValidationErr);
                     $e = save_update_Block($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        try{
                            $e->setLocale('fr_CA');
                            if(isset($data['BlockI18n_Text_frCA'])){$e->setText($data['BlockI18n_Text_frCA']);}
                            if(isset($data['BlockI18n_Version_frCA'])){$e->setVersion($data['BlockI18n_Version_frCA']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new BlockI18n();
                            $mt->setLocale('fr_CA');
                            
                            if(isset($data['BlockI18n_Text_frCA'])){$mt->setText($data['BlockI18n_Text_frCA']);}
                            if(isset($data['BlockI18n_Version_frCA'])){$mt->setVersion($data['BlockI18n_Version_frCA']);}
                            $e->addBlockI18n($mt);$e->save();
                        }
                    
                        try{
                            $e->setLocale('en_US');
                            if(isset($data['BlockI18n_Text_enUS'])){$e->setText($data['BlockI18n_Text_enUS']);}
                            if(isset($data['BlockI18n_Version_enUS'])){$e->setVersion($data['BlockI18n_Version_enUS']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new BlockI18n();
                            $mt->setLocale('en_US');
                            
                            if(isset($data['BlockI18n_Text_enUS'])){$mt->setText($data['BlockI18n_Text_enUS']);}
                            if(isset($data['BlockI18n_Version_enUS'])){$mt->setVersion($data['BlockI18n_Version_enUS']);}
                            $e->addBlockI18n($mt);$e->save();
                        }
                    
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Block'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Block'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_Block($data, $request);
                    
                    
                        
                        
                        
                    $e->setLocale('fr_CA');$e->setText($data['BlockI18n_Text_frCA']);
                    $e->setLocale('fr_CA');$e->setVersion($data['BlockI18n_Version_frCA']);
                    $e->setLocale('en_US');$e->setText($data['BlockI18n_Text_enUS']);
                    $e->setLocale('en_US');$e->setVersion($data['BlockI18n_Version_enUS']);
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        $e->save();
                        
                        unset($_SESSION['memoire']['onglet']['Block']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Block']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Block']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Block']['i'] = set_var($_SESSION['memoire']['onglet']['Block']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Block'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Block';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        ...
                        if(!$data['IdContent'] && !$request['ip'] or ($request['je'] =='BlockTable' and !$request['ip']) ){
                            $output['onReadyJs'] .= "$('#formMsBlock #msBlockBt').click();".$closeDiag."";
                        }else{
                            $output['onReadyJs'] .= "$('.child_pannel [p=\"Block\"]').click();".$closeDiag."";
                        }
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new BlockForm();
                                $e->Block['request'] = $request;
                                $e->Block['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                if(!empty($request['ip'])){$f->Block['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Block']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Block']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdBlock';
                    ...
                    unset($_SESSION['memoire']['onglet']['Block']['ixmemautoc']);
                }
                ...
                $IdCurrentBlock = $data['i'];
                
            $q = BlockQuery::create()->filterByIdBlock($IdCurrentBlock)->setFormatter(ModelCriteria::FORMAT_ARRAY)->findOne();

            @$output['onReadyJs'] .= "
                $('#formBlock .divtr.readonly .divtd[in=\"inSlug\"] label.readonly').text('".addslashes($q['Slug'])."');
                $('#formBlock #Slug').val('".addslashes($q['Slug'])."');
                $('#formBlock .divtr.readonly .divtd[in=\"inStatus\"] label.readonly').text('".addslashes($q['Status'])."');
                $('#formBlock #Status').val('".addslashes($q['Status'])."');
                $('#formBlock .divtr.readonly .divtd[in=\"inIdBlock\"] label.readonly').text('".addslashes($q['IdBlock'])."');
                $('#formBlock #IdBlock').val('".addslashes($q['IdBlock'])."');
            ";
            ...
            case 'delete':
                ...
                $obj = BlockQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Block';
                if(!$obj->delete()){
        ...
        case 'getBlockFileList':
            if($_SESSION[_AUTH_VAR]->hasRights("BlockBlockFile",'r')){
                $listChild=true;

                $_SESSION['memoire']['onglet']['Block']['child']['list'][$request['ip']]= 'BlockFile';
                if(!empty($request['ip'])){$IdCurrentBlock = $request['ip'];}
                $IdSplit=""; 
                if(isset($request['ms']) and $request['ms'] == 'clear'){
                    $request['ms'] = '';$request['order'] = '';
                    unset($_SESSION['memoire']['search']['formMsBlockBlockFile'.$IdSplit]);
                    unset($_SESSION['memoire']['search']['formOrderBlockBlockFile']);
                    unset($_SESSION['memoire']['onglet']['BlockBlockFile']['pg']);
                    
                }
                if(!empty($request['order'])){
                    $search['order'] = json_decode($request['order'], true);
                    $_SESSION['memoire']['search']['formOrderBlockBlockFile'][$search['order']['col']] = $search['order']['sens'];
                    $search['order']= $_SESSION['memoire']['search']['formOrderBlockBlockFile'];
                }else if(!empty($_SESSION['memoire']['search']['formOrderBlockBlockFile'])){
                    $search['order']=$_SESSION['memoire']['search']['formOrderBlockBlockFile'];
                }
                if(!empty($request['ms'])){
                    parse_str($request['ms'], $search['ms']);
                    if($_SESSION['memoire']['search']['formMsBlockBlockFile'.$IdSplit] != $request['ms']){
                        unset($_SESSION['memoire']['onglet']['BlockBlockFile']['pg']);
                    }
                    $_SESSION['memoire']['search']['formMsBlockBlockFile'.$IdSplit] = $request['ms'];
                }else if(!empty($_SESSION['memoire']['search']['formMsBlockBlockFile'.$IdSplit])){
                    parse_str($_SESSION['memoire']['search']['formMsBlockBlockFile'.$IdSplit], $search['ms']);
                }
                if(!empty($request['pg'])){ $_SESSION['memoire']['onglet']['BlockBlockFile']['pg'] = $request['pg'];}
                
                $f = new BlockForm();
                $f->BlockFile['request'] = $request;
                if(!empty($request['ip'])){
                    $f->BlockFile['parentId'] = $request['ip'];
                    $data['IdBlock'] = $request['ip'];
                }
                
                if(!empty($request['noHeader'])){$params['noHeader'] = $request['noHeader'];}
                $_SESSION['CurrentParent'] = 'Block';
                $childTableData = $f->getBlockFileList(@$request['ip'],$request['pg'], @$request['ui'], @$request['pui'], true, @$search, @$params);
                
                unset($_SESSION['CurrentParent']);
                @$output['html'] .= div(@$f->CcBlockFileTopList.$childTableData['html'].@$f->CcBlockFileBottomList, 'BlockFileTableCntnr','class="child-wrapper"');
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Block",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Block",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Block",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Block",'r')){
            init_list_base($request,$search,'Block','','','','');
            $e = new BlockForm();
            $e->Block['request'] = $request;
            if(!empty($request['ip'])){$e->Block['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderBlock']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsBlock'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Block($data, $request=array()){
        $e = new BlockForm();
        $e->Block['request'] = $request;
        return $e->save_update_Block($data);
    }
    function save_create_Block($data, $request=array()){
        $e = new BlockForm();
        $e->Block['request'] = $request;
        return $e->save_create_Block($data);
    }
