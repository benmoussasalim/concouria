    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = ConcoursFileQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new ConcoursFile();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        case 'ForceDl':
            if($_SESSION[_AUTH_VAR]->hasRights('ConcoursFile','r')
            || $_SESSION[_AUTH_VAR]->hasRights($request['pc']."ConcoursFile",'r')
            ){
                $dataObj= ConcoursFileQuery::create()->findPk($request['i']);
...
array('name'=>$_FILES["file"]["name"][$f],'size'=>$_FILES["file"]["size"][$f],'type'=>$_FILES["file"]["type"][$f],'tmp_name'=>$_FILES["file"]["tmp_name"][$f],'error'=>$_FILES["file"]["error"][$f]);
                    ...
                    if( $path_info["extension"] ){
                        ...
                        if($tabIp){
                            foreach(array_unique($tabIp) as $ip){
                                $e = save_create_ConcoursFile($data, $request);
                                $e->setIdConcours($ip);
                                $path_rep_fichier = 'mod/file/';
                                $path_fichier = 'mod/file/ConcoursFile/';
                                ...
                                if(@$data['error'] == ''){
                                    if($_GET['blob']==1){
                                        ...
                                        if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'ConcoursFile';}
                                        $e->fromArray($data);
                                        if ($e->validate()){
                                            $e->save();
            ...
            case 'save':
                ...
                if(!empty($pkPresent)){
                    ...
                    ## Save
                     ...
                     $e = save_update_ConcoursFile($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Photo'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Photo'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['ConcoursFile']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['ConcoursFile']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['ConcoursFile']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['ConcoursFile']['i'] = set_var($_SESSION['memoire']['onglet']['ConcoursFile']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);

                        }
                        
                        
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $data['IdConcours'] = json_encode($e->getIdConcours());
                        
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Photo'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'ConcoursFile';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        ...
                        if(!$data['IdConcours'] && !$request['ip'] or ($request['je'] =='ConcoursFileTable' and !$request['ip']) ){
                            $output['onReadyJs'] .= "$('#formMsConcoursFile #msConcoursFileBt').click();".$closeDiag."";
                        }else{
                            $output['onReadyJs'] .= "$('.child_pannel [p=\"ConcoursFile\"]').click();".$closeDiag."";
                        }
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new ConcoursFileForm();
                                $e->ConcoursFile['request'] = $request;
                                $e->ConcoursFile['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new ConcoursFileForm();
                $f->ConcoursFile['request'] = $request;
                if(!empty($request['ip'])){$f->ConcoursFile['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['ConcoursFile']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['ConcoursFile']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdConcoursFile';
                    ...
                    unset($_SESSION['memoire']['onglet']['ConcoursFile']['ixmemautoc']);
                }
                ...
                $IdCurrentConcoursFile = $data['i'];
                
            $q = ConcoursFileQuery::create()->filterByIdConcoursFile($IdCurrentConcoursFile)->setFormatter(ModelCriteria::FORMAT_ARRAY)->findOne();

            @$output['onReadyJs'] .= "
                $('#formConcoursFile .divtr.readonly .divtd[in=\"inSize\"] label.readonly').text('".addslashes($q['Size'])."');
                $('#formConcoursFile #Size').val('".addslashes($q['Size'])."');
                $('#formConcoursFile .divtr.readonly .divtd[in=\"inName\"] label.readonly').text('".addslashes($q['Name'])."');
                $('#formConcoursFile #Name').val('".addslashes($q['Name'])."');
            ";
            ...
            case 'delete':
                ...
                $obj = ConcoursFileQuery::create()->findPk($request['i']);
        ...
        if(is_file(_INSTALL_PATH.$obj->getFichier())){
            unlink(_INSTALL_PATH.$obj->getFichier());
                ...
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'ConcoursFile';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."ConcoursFile",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."ConcoursFile",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."ConcoursFile",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("ConcoursFile",'r')){
            init_list_base($request,$search,'ConcoursFile','','','','');
            $e = new ConcoursFileForm();
            $e->ConcoursFile['request'] = $request;
            if(!empty($request['ip'])){$e->ConcoursFile['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderConcoursFile']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsConcoursFile'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_ConcoursFile($data, $request=array()){
        $e = new ConcoursFileForm();
        $e->ConcoursFile['request'] = $request;
        return $e->save_update_ConcoursFile($data);
    }
    function save_create_ConcoursFile($data, $request=array()){
        $e = new ConcoursFileForm();
        $e->ConcoursFile['request'] = $request;
        return $e->save_create_ConcoursFile($data);
    }
