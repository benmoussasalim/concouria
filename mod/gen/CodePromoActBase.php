...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = CodePromoQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new CodePromo();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'CodePromo';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_CodePromo($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Code Promotion'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Code Promotion'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_CodePromo($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['CodePromo']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['CodePromo']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['CodePromo']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['CodePromo']['i'] = set_var($_SESSION['memoire']['onglet']['CodePromo']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Code Promotion'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'CodePromo';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdCodePromo'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new CodePromoForm();
                                $e->CodePromo['request'] = $request;
                                $e->CodePromo['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new CodePromoForm();
                $f->CodePromo['request'] = $request;
                if(!empty($request['ip'])){$f->CodePromo['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['CodePromo']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['CodePromo']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdCodePromo';
                    ...
                    unset($_SESSION['memoire']['onglet']['CodePromo']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = CodePromoQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'CodePromo';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."CodePromo",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."CodePromo",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."CodePromo",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("CodePromo",'r')){
            init_list_base($request,$search,'CodePromo','','','','');
            $e = new CodePromoForm();
            $e->CodePromo['request'] = $request;
            if(!empty($request['ip'])){$e->CodePromo['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderCodePromo']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsCodePromo'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_CodePromo($data, $request=array()){
        $e = new CodePromoForm();
        $e->CodePromo['request'] = $request;
        return $e->save_update_CodePromo($data);
    }
    function save_create_CodePromo($data, $request=array()){
        $e = new CodePromoForm();
        $e->CodePromo['request'] = $request;
        return $e->save_create_CodePromo($data);
    }
