...
###############################
/**
 * Form class on the 'FaqI18n' table.
 *
 */
class FaqI18nForm extends FaqI18n{
public $tableName="FaqI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('FaqI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."FaqI18n/edit/", "id='addFaqI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addFaqI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['FaqI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['FaqI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'FaqI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'FaqI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#FaqI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#FaqI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('FaqI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('FaqI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('FaqI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteFaqI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='FaqI18nRow".$data->getPrimaryKey()."'
                    data-table='FaqI18n' 
            ...
            $tr .= input('hidden', 'rowCountFaqI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'FaqI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='FaqI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
        ...
        if(empty($hasParent)){
            $editEvent = "
        $(\"#FaqI18nListForm [j='deleteFaqI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msFaqI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msFaqI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#FaqI18nListForm tr[ecf=1] td[j='editFaqI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."FaqI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."FaqI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#FaqI18nListForm [j='deleteFaqI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msFaqI18nBt\').length>0){ $(\'#msFaqI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'FaqI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntFaqI18nListForm #addFaqI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'FaqI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#FaqI18nListForm tr[ecf=1] td[j='editFaqI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'FaqI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('FaqI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#FaqI18nListForm [j='deleteFaqI18n']\").unbind('click');
        $('#FaqI18nListForm #addFaqI18n').unbind('click');
        $(\"#FaqI18nListForm tr[ecf=1] td[j='editFaqI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#FaqI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('FaqI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('FaqI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_FaqI18n($data){
    ...
    public function save_update_FaqI18n($data){

        
        $e = FaqI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of FaqI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['FaqI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['FaqI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addFaqI18n_child').bind('click.addFaqI18n', function (){
                    $.post('"._SITE_URL."mod/act/FaqI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addFaqI18n_child').bind('click.addFaqI18n', function (){
                document.location= '"._SITE_URL."FaqI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'FaqI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'FaqI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formFaqI18n #saveFaqI18n').removeAttr('disabled');
                    $('#formFaqI18n #saveFaqI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formFaqI18n #saveFaqI18n').css('cursor', 'default');
                    if($('#formFaqI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formFaqI18n #saveFaqI18n').bind('click.saveFaqI18n', function (data){
                    $('#formFaqI18n #saveFaqI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formFaqI18n #saveFaqI18n').css('cursor', 'progress');
                    $('#formFaqI18n #saveFaqI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formFaqI18n .tinymce').each(function(index) {
                        eval(' $(\"#formFaqI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formFaqI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formFaqI18n\").find(\"[s='d']\").serializeArray();
                        $('#formFaqI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formFaqI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/FaqI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formFaqI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formFaqI18n #formChangedFaqI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formFaqI18n #saveFaqI18n').unbind('click.saveFaqI18n');
                $('#formFaqI18n #saveFaqI18n').remove();";
        }
        
        if($dataObj == null){
            $this->FaqI18n['isNew'] = 'yes';
        }
        ...
        if(!isset($params['ChildHide'])  and !isset($this->FaqI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdFaq()){
            $this->printLink =
                ...
                ,'FaqI18nControls', "class='custom-controls'");
        ...
        $this->formTitle = p(href("",_SITE_URL.'FaqI18n').$titre_form_str, 'class="breadcrumb"'); 
        $this->add_button= '';
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            
            $this->add_button = 
                
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addFaqI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'FaqI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formFaqI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['FaqI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['FaqI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['FaqI18n']['Start']['html']
                
                
                
                .@$this->fields['FaqI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntFaqI18n", "class='divStdform' CntTabs=1 ")
        , "id='formFaqI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdFaq()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelFaqI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntFaqI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['FaqI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['FaqI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['FaqI18n']['ixmemautocapp']);
        }
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntFaqI18n');},500); 
    ".$toggleForm."
    bind_form('FaqI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['FaqI18n'] as $field=>$ar){
                $this->fields['FaqI18n'][$field]['html'] = $this->fieldsRo['FaqI18n'][$field]['html'];
            }
    ...
    */
    ...
    $this->ajaxPageAct = 'FaqI18nAct.php';
