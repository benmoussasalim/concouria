...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = GrpTaxeQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new GrpTaxe();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'GrpTaxe';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_GrpTaxe($data, $request);
                    
                        if(empty($data['CalcId'])){
                            $e->setCalcId(bcadd("020", $e->getIdGroupTaxeSup()));
                        }
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Groupe de taxe'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Groupe de taxe'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_GrpTaxe($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['GrpTaxe']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['GrpTaxe']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['GrpTaxe']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['GrpTaxe']['i'] = set_var($_SESSION['memoire']['onglet']['GrpTaxe']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Groupe de taxe'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'GrpTaxe';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdGroupTaxeSup'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new GrpTaxeForm();
                                $e->GrpTaxe['request'] = $request;
                                $e->GrpTaxe['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new GrpTaxeForm();
                $f->GrpTaxe['request'] = $request;
                if(!empty($request['ip'])){$f->GrpTaxe['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['GrpTaxe']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['GrpTaxe']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdGroupTaxeSup';
                    ...
                    unset($_SESSION['memoire']['onglet']['GrpTaxe']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = GrpTaxeQuery::create()->findPk($request['i']);
                
            if($obj->countProvinces()){ $error = handleNotOkResponse(_("Cette entrée ne peut être supprimée car elle est présentement utilisée dans")." 'Province'. ", '', true,'Groupe de taxe'); die( $error['onReadyJs'] ); }
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'GrpTaxe';
                if(!$obj->delete()){
        ...
        case 'getTaxeList':
            if($_SESSION[_AUTH_VAR]->hasRights("GrpTaxeTaxe",'r')){
                $listChild=true;

                $_SESSION['memoire']['onglet']['GrpTaxe']['child']['list'][$request['ip']]= 'Taxe';
                if(!empty($request['ip'])){$IdCurrentGrpTaxe = $request['ip'];}
                $IdSplit=""; 
                if(isset($request['ms']) and $request['ms'] == 'clear'){
                    $request['ms'] = '';$request['order'] = '';
                    unset($_SESSION['memoire']['search']['formMsGrpTaxeTaxe'.$IdSplit]);
                    unset($_SESSION['memoire']['search']['formOrderGrpTaxeTaxe']);
                    unset($_SESSION['memoire']['onglet']['GrpTaxeTaxe']['pg']);
                    
                }
                if(!empty($request['order'])){
                    $search['order'] = json_decode($request['order'], true);
                    $_SESSION['memoire']['search']['formOrderGrpTaxeTaxe'][$search['order']['col']] = $search['order']['sens'];
                    $search['order']= $_SESSION['memoire']['search']['formOrderGrpTaxeTaxe'];
                }else if(!empty($_SESSION['memoire']['search']['formOrderGrpTaxeTaxe'])){
                    $search['order']=$_SESSION['memoire']['search']['formOrderGrpTaxeTaxe'];
                }
                if(!empty($request['ms'])){
                    parse_str($request['ms'], $search['ms']);
                    if($_SESSION['memoire']['search']['formMsGrpTaxeTaxe'.$IdSplit] != $request['ms']){
                        unset($_SESSION['memoire']['onglet']['GrpTaxeTaxe']['pg']);
                    }
                    $_SESSION['memoire']['search']['formMsGrpTaxeTaxe'.$IdSplit] = $request['ms'];
                }else if(!empty($_SESSION['memoire']['search']['formMsGrpTaxeTaxe'.$IdSplit])){
                    parse_str($_SESSION['memoire']['search']['formMsGrpTaxeTaxe'.$IdSplit], $search['ms']);
                }
                if(!empty($request['pg'])){ $_SESSION['memoire']['onglet']['GrpTaxeTaxe']['pg'] = $request['pg'];}
                
                $f = new GrpTaxeForm();
                $f->Taxe['request'] = $request;
                if(!empty($request['ip'])){
                    $f->Taxe['parentId'] = $request['ip'];
                    $data['IdGroupTaxeSup'] = $request['ip'];
                }
                
                if(!empty($request['noHeader'])){$params['noHeader'] = $request['noHeader'];}
                $_SESSION['CurrentParent'] = 'GrpTaxe';
                $childTableData = $f->getTaxeList(@$request['ip'],$request['pg'], @$request['ui'], @$request['pui'], true, @$search, @$params);
                
                unset($_SESSION['CurrentParent']);
                @$output['html'] .= div(@$f->CcTaxeTopList.$childTableData['html'].@$f->CcTaxeBottomList, 'TaxeTableCntnr','class="child-wrapper"');
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."GrpTaxe",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."GrpTaxe",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."GrpTaxe",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("GrpTaxe",'r')){
            init_list_base($request,$search,'GrpTaxe','','','','');
            $e = new GrpTaxeForm();
            $e->GrpTaxe['request'] = $request;
            if(!empty($request['ip'])){$e->GrpTaxe['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderGrpTaxe']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsGrpTaxe'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_GrpTaxe($data, $request=array()){
        $e = new GrpTaxeForm();
        $e->GrpTaxe['request'] = $request;
        return $e->save_update_GrpTaxe($data);
    }
    function save_create_GrpTaxe($data, $request=array()){
        $e = new GrpTaxeForm();
        $e->GrpTaxe['request'] = $request;
        return $e->save_create_GrpTaxe($data);
    }
