...
###############################
/**
 * Form class on the 'RegionI18n' table.
 *
 */
class RegionI18nForm extends RegionI18n{
public $tableName="RegionI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('RegionI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."RegionI18n/edit/", "id='addRegionI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addRegionI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['RegionI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['RegionI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'RegionI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'RegionI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#RegionI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#RegionI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('RegionI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('RegionI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('RegionI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteRegionI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='RegionI18nRow".$data->getPrimaryKey()."'
                    data-table='RegionI18n' 
            ...
            $tr .= input('hidden', 'rowCountRegionI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'RegionI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='RegionI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
        ...
        if(empty($hasParent)){
            $editEvent = "
        $(\"#RegionI18nListForm [j='deleteRegionI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msRegionI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msRegionI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#RegionI18nListForm tr[ecf=1] td[j='editRegionI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."RegionI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."RegionI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#RegionI18nListForm [j='deleteRegionI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msRegionI18nBt\').length>0){ $(\'#msRegionI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'RegionI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntRegionI18nListForm #addRegionI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'RegionI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#RegionI18nListForm tr[ecf=1] td[j='editRegionI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'RegionI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('RegionI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#RegionI18nListForm [j='deleteRegionI18n']\").unbind('click');
        $('#RegionI18nListForm #addRegionI18n').unbind('click');
        $(\"#RegionI18nListForm tr[ecf=1] td[j='editRegionI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#RegionI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('RegionI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('RegionI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_RegionI18n($data){
    ...
    public function save_update_RegionI18n($data){

        
        $e = RegionI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of RegionI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['RegionI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['RegionI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addRegionI18n_child').bind('click.addRegionI18n', function (){
                    $.post('"._SITE_URL."mod/act/RegionI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addRegionI18n_child').bind('click.addRegionI18n', function (){
                document.location= '"._SITE_URL."RegionI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'RegionI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'RegionI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formRegionI18n #saveRegionI18n').removeAttr('disabled');
                    $('#formRegionI18n #saveRegionI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formRegionI18n #saveRegionI18n').css('cursor', 'default');
                    if($('#formRegionI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formRegionI18n #saveRegionI18n').bind('click.saveRegionI18n', function (data){
                    $('#formRegionI18n #saveRegionI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formRegionI18n #saveRegionI18n').css('cursor', 'progress');
                    $('#formRegionI18n #saveRegionI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formRegionI18n .tinymce').each(function(index) {
                        eval(' $(\"#formRegionI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formRegionI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formRegionI18n\").find(\"[s='d']\").serializeArray();
                        $('#formRegionI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formRegionI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/RegionI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formRegionI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formRegionI18n #formChangedRegionI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formRegionI18n #saveRegionI18n').unbind('click.saveRegionI18n');
                $('#formRegionI18n #saveRegionI18n').remove();";
        }
        
        if($dataObj == null){
            $this->RegionI18n['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new RegionI18n();
            $this->RegionI18n['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->RegionI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdRegion()){
            $this->printLink =
                ...
                ,'RegionI18nControls', "class='custom-controls'");
        ...
        $this->formTitle = p(href("",_SITE_URL.'RegionI18n').$titre_form_str, 'class="breadcrumb"'); 
        $this->add_button= '';
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            
            $this->add_button = 
                
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addRegionI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'RegionI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formRegionI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['RegionI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['RegionI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['RegionI18n']['Start']['html']
                
                
                
                .@$this->fields['RegionI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntRegionI18n", "class='divStdform' CntTabs=1 ")
        , "id='formRegionI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdRegion()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelRegionI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntRegionI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['RegionI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['RegionI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['RegionI18n']['ixmemautocapp']);
        }
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntRegionI18n');},500); 
    ".$toggleForm."
    bind_form('RegionI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['RegionI18n'] as $field=>$ar){
                $this->fields['RegionI18n'][$field]['html'] = $this->fieldsRo['RegionI18n'][$field]['html'];
            }
    ...
    */
    ...
    $this->ajaxPageAct = 'RegionI18nAct.php';
