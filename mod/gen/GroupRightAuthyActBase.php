...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = GroupRightAuthyQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new GroupRightAuthy();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'GroupRightAuthy';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_GroupRightAuthy($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Group'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Group'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_GroupRightAuthy($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['GroupRightAuthy']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['GroupRightAuthy']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['GroupRightAuthy']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['GroupRightAuthy']['i'] = set_var($_SESSION['memoire']['onglet']['GroupRightAuthy']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Group'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'GroupRightAuthy';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        ...
                        if(!$data['IdAuthy'] && !$request['ip'] or ($request['je'] =='GroupRightAuthyTable' and !$request['ip']) ){
                            $output['onReadyJs'] .= "$('#formMsGroupRightAuthy #msGroupRightAuthyBt').click();".$closeDiag."";
                        }else{
                            $output['onReadyJs'] .= "$('.child_pannel [p=\"GroupRightAuthy\"]').click();".$closeDiag."";
                        }
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new GroupRightAuthyForm();
                                $e->GroupRightAuthy['request'] = $request;
                                $e->GroupRightAuthy['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new GroupRightAuthyForm();
                $f->GroupRightAuthy['request'] = $request;
                if(!empty($request['ip'])){$f->GroupRightAuthy['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['GroupRightAuthy']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['GroupRightAuthy']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    ...
                    unset($_SESSION['memoire']['onglet']['GroupRightAuthy']['ixmemautoc']);
                }
                ...
                $IdCurrentGroupRightAuthy = $data['i'];
            ...
            case 'delete':
                ...
                $obj = GroupRightAuthyQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'GroupRightAuthy';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."GroupRightAuthy",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."GroupRightAuthy",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."GroupRightAuthy",'r')
            ...
            if(!empty($request['ip'])){
                $relData['IdAuthy'] = $request['ip'];
                /* pour savoir qu'elle est l'ip dans l'action ajouter */
                $relData['ip'] = json_decode($request['ip']);
                $relData['pc'] = $request['pc'];
            }
            
            $tab_id = json_decode($request['i']);
            if($request['ip']){
                if($tab_id[0] and $request['ip']!=$tab_id[0] )
                    $request['ip'] =$tab_id[0];
                $request['i'] = '['.$request['ip'].','.$tab_id[1].']';
                /* verifie s'il existe deja sinon le cree */
                $obj_cr = GroupRightAuthyQuery::create()->setFormatter(ModelCriteria::FORMAT_ARRAY)->findPk(json_decode($request['i']));
                if(!$obj_cr['IdAuthy']){
                    $arr = json_decode($request['i']);

                    $data_cre['IdAuthy'] = $arr[0];
                    $data_cre['IdGroupRight'] = $arr[1];
                    
                    $e = new GroupRightAuthy();
                    $e->setPrimaryKey(json_decode($request['i']));
                    
                    $e->fromArray($data_cre );
                    $e->save();
                }
            }
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("GroupRightAuthy",'r')){
            init_list_base($request,$search,'GroupRightAuthy','','','','');
            $e = new GroupRightAuthyForm();
            $e->GroupRightAuthy['request'] = $request;
            if(!empty($request['ip'])){$e->GroupRightAuthy['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderGroupRightAuthy']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsGroupRightAuthy'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_GroupRightAuthy($data, $request=array()){
        $e = new GroupRightAuthyForm();
        $e->GroupRightAuthy['request'] = $request;
        return $e->save_update_GroupRightAuthy($data);
    }
    function save_create_GroupRightAuthy($data, $request=array()){
        $e = new GroupRightAuthyForm();
        $e->GroupRightAuthy['request'] = $request;
        return $e->save_create_GroupRightAuthy($data);
    }
