...
###############################
/**
 * Form class on the 'ContentI18nVersion' table.
 *
 */
class ContentI18nVersionForm extends ContentI18nVersion{
public $tableName="ContentI18nVersion";
...
public $siteTitle;
    ...
    public function getListSearch($IdParent='', $search=''){
        ...
        $q = ContentI18nVersionQuery::create();
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('ContentI18nVersion', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."ContentI18nVersion/edit/", "id='addContentI18nVersion' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addContentI18nVersionAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['ContentI18nVersion']['pg'])){
            $page = $_SESSION['memoire']['onglet']['ContentI18nVersion']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'ContentI18nVersionAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'ContentI18nVersionAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#ContentI18nVersionListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#ContentI18nVersionListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('ContentI18nVersion', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('ContentI18nVersion', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('ContentI18nVersion', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteContentI18nVersion' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='ContentI18nVersionRow".$data->getPrimaryKey()."'
                    data-table='ContentI18nVersion' 
            ...
            $tr .= input('hidden', 'rowCountContentI18nVersion', $i);
                ...
                $pagerRow =
                    div(
                        ...
                        .div(
                            ...
                            ,'','id="ContentI18nVersionPagination"')
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'ContentI18nVersionControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='ContentI18nVersionTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
                ,'listForm',' class="ac-list contentHolderListP" ')
                .$this->CcToListBottom
                .@$bottomRow
            , 'ContentI18nVersionListForm');
        
        $hasParent = json_decode($IdParent);
        if(empty($hasParent)){
            $editEvent = "
        $(\"#ContentI18nVersionListForm [j='deleteContentI18nVersion']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msContentI18nVersionBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msContentI18nVersionBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#ContentI18nVersionListForm tr[ecf=1] td[j='editContentI18nVersion']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."ContentI18nVersion/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."ContentI18nVersion/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#ContentI18nVersionListForm [j='deleteContentI18nVersion']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msContentI18nVersionBt\').length>0){ $(\'#msContentI18nVersionBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'ContentI18nVersionTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntContentI18nVersionListForm #addContentI18nVersion').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'ContentI18nVersionTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#ContentI18nVersionListForm tr[ecf=1] td[j='editContentI18nVersion']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'ContentI18nVersionTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('ContentI18nVersion','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#ContentI18nVersionListForm [j='deleteContentI18nVersion']\").unbind('click');
        $('#ContentI18nVersionListForm #addContentI18nVersion').unbind('click');
        $(\"#ContentI18nVersionListForm tr[ecf=1] td[j='editContentI18nVersion']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#ContentI18nVersionListForm [j='button']\").unbind();   
        pagination_sorted_bind('ContentI18nVersion','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('ContentI18nVersion','".addslashes($variableAutoc)."');
    ...
    public function save_create_ContentI18nVersion($data){

        unset($data['IdContentI18nVersion']);
        $e = new ContentI18nVersion();
        ...
        $e->setText(($data['Text']=='')?null:$data['Text']);
    ...
    public function save_update_ContentI18nVersion($data){

        
        $e = ContentI18nVersionQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of ContentI18nVersion
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['ContentI18nVersion']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['ContentI18nVersion']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addContentI18nVersion_child').bind('click.addContentI18nVersion', function (){
                    $.post('"._SITE_URL."mod/act/ContentI18nVersionAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addContentI18nVersion_child').bind('click.addContentI18nVersion', function (){
                document.location= '"._SITE_URL."ContentI18nVersion/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'ContentI18nVersion', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'ContentI18nVersion','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formContentI18nVersion #saveContentI18nVersion').removeAttr('disabled');
                    $('#formContentI18nVersion #saveContentI18nVersion').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formContentI18nVersion #saveContentI18nVersion').css('cursor', 'default');
                    if($('#formContentI18nVersion #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formContentI18nVersion #saveContentI18nVersion').bind('click.saveContentI18nVersion', function (data){
                    $('#formContentI18nVersion #saveContentI18nVersion').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formContentI18nVersion #saveContentI18nVersion').css('cursor', 'progress');
                    $('#formContentI18nVersion #saveContentI18nVersion').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formContentI18nVersion .tinymce').each(function(index) {
                        eval(' $(\"#formContentI18nVersion #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formContentI18nVersion select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formContentI18nVersion\").find(\"[s='d']\").serializeArray();
                        $('#formContentI18nVersion select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formContentI18nVersion\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/ContentI18nVersionAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formContentI18nVersion #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formContentI18nVersion #formChangedContentI18nVersion').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formContentI18nVersion #saveContentI18nVersion').unbind('click.saveContentI18nVersion');
                $('#formContentI18nVersion #saveContentI18nVersion').remove();";
        }
        
        if($dataObj == null){
            $this->ContentI18nVersion['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new ContentI18nVersion();
            $this->ContentI18nVersion['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->ContentI18nVersion['request']['ChildHide']) ){
            
        }
        
        
        if($dataObj->getIdContentI18nVersion()){
            $this->printLink =
                div(
                    ""
                    .@$this->CcPrintLinkDiv
                ...
                ,'ContentI18nVersionControls', "class='custom-controls'");
        ...
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            ...
            $this->add_button = 
                ...
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addContentI18nVersion_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'ContentI18nVersionControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formContentI18nVersion');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['ContentI18nVersion']['tog']) and 
            $_SESSION['memoire']['onglet']['ContentI18nVersion']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['ContentI18nVersion']['Start']['html']
                
                
                
                .@$this->fields['ContentI18nVersion']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntContentI18nVersion", "class='divStdform' CntTabs=1 ")
        , "id='formContentI18nVersion' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdContentI18nVersion()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelContentI18nVersion', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntContentI18nVersionChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['ContentI18nVersion']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['ContentI18nVersion']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['ContentI18nVersion']['ixmemautocapp']);
        }
    ...
    ".$jqueryDatePicker."
    bind_ui_active('ContentI18nVersion');
    ".$this->AddButtonJs."
    ".$this->SaveButtonJs."
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntContentI18nVersion');},500); 
    ".$toggleForm."
    bind_form('ContentI18nVersion','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['ContentI18nVersion'] as $field=>$ar){
                $this->fields['ContentI18nVersion'][$field]['html'] = $this->fieldsRo['ContentI18nVersion'][$field]['html'];
            }
    ...
    */
    public function getSelectOptions($IdParent='', $selected='',$arrayOnly=false){
        ...
        if(!empty($IdParent)){
             ...
             $q->select(array('Name', 'IdContentI18nVersion'));
    ...
    $this->ajaxPageAct = 'ContentI18nVersionAct.php';
