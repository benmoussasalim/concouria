...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            case 'head':
            ###### TH
                $trHead = @$this->cCmoreColsHeaderFirst.th(_("Titre"), " th='sorted'  rcColone='Title' c='Title'  ").th(_("Ordre du menu"), " th='sorted'  rcColone='Order' c='Order'  "). $this->cCmoreColsHeader;
        ...
        @$this->fieldsSearch['Menu']['Title'] = div(input('text', 'Title', $this->searchMs['Title'], ' othertabs=1  placeholder="'._('Nom').'"',''),'','class="ac-search-item ms-Title"');
            
        $AddShortcut =button(span(_("Ajouter aux raccourcis"),'data-tooltip="'._('Ajouter aux raccourcis').'"'),'class="icon shortcut" title="'._('Ajouter aux raccourcis').'" id="msMenuAddShortcut"');
        if(!empty($this->search['Autoc']['SelId'])){$AddShortcut="";}

        $this->fieldsSearch['Menu']['Button'] = 
            $this->CcToSearchMsForm
            .div(
               button(span(_("Rechercher"),'data-tooltip="'._('Rechercher').'"'),'id="msMenuBt" title="'._('Lancer la recherche').'" class="icon search"')
               .button(span(_("Vider"),'data-tooltip="'._('Vider la recherche').'"'),' title="'._('Vider la recherche').'" id="msMenuBtClear" class="icon clear"')
               .@$this->fieldsSearch['Menu']['BShortEnd']
               .@$AddShortcut
               .@$this->fieldsSearch['Menu']['BEnd']
            ,'','class="ac-action-buttons"');
                 
                $trSearch = ''
            .div(
                button(span(_("Afficher la recherche")),'class="js-toggle-MenuSearch btn-toggle-search" data-toggle="'._("Cacher la recherche").'"')
                .form(@$this->fieldsSearch['Menu']['Start'].
                    $this->fieldsSearch['Menu']['Title'].
                @$this->fieldsSearch['Menu']['End'].
            $this->fieldsSearch['Menu']['Button']
            ,"id='formMsMenu' CntTabs=1 ")
            ,"", "  class='msSearchCtnr'");
                return $trSearch;
            break;
            case 'add':
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('Menu', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."Menu/edit/", "id='addMenu' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addMenuAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        $this->siteTitle .=_("Liste Hiérarchie");
        ...
        if(!empty($_SESSION['memoire']['onglet']['Menu']['pg'])){
            $page = $_SESSION['memoire']['onglet']['Menu']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'MenuAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'MenuAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#MenuListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#MenuListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('Menu', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('Menu', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('Menu', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteMenu' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                ;
                 ...
                 $titreRaccour =''; $titreRaccour .= _('Hiérarchie');
                ...
                */
                ...
                @$tr .= 
                tr(
                    @$cCmoreColsFirst.td(span(strip_tags((isset($altValue['Title']) and $altValue['Title'])?$altValue['Title']:$data->getTitle()." "),'class="ac-list-td-content"'),"  data-responsive=\""._("Titre").":\" j='editMenu'  i='".json_encode($data->getPrimaryKey())."' c='Title'  ")
                            .td(span(strip_tags((isset($altValue['Order']) and $altValue['Order'])?$altValue['Order']:$data->getOrder()." "),'class="ac-list-td-content"'),"  data-responsive=\""._("Ordre du menu").":\" j='editMenu'  i='".json_encode($data->getPrimaryKey())."' c='Order'  ")
            ...
            $tr .= input('hidden', 'rowCountMenu', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'MenuControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='MenuTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
        ...
        if(empty($hasParent)){
            $editEvent = "
        $(\"#MenuListForm [j='deleteMenu']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_('Hiérarchie'))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msMenuBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msMenuBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#MenuListForm tr[ecf=1] td[j='editMenu']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."Menu/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."Menu/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#MenuListForm [j='deleteMenu']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_('Hiérarchie'))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msMenuBt\').length>0){ $(\'#msMenuBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'MenuTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntMenuListForm #addMenu').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'MenuTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#MenuListForm tr[ecf=1] td[j='editMenu']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'MenuTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('Menu','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#MenuListForm [j='deleteMenu']\").unbind('click');
        $('#MenuListForm #addMenu').unbind('click');
        $(\"#MenuListForm tr[ecf=1] td[j='editMenu']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#MenuListForm [j='button']\").unbind();   
        pagination_sorted_bind('Menu','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('Menu','".addslashes($variableAutoc)."');
        
        
        
        
        bind_button_list('Menu','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."',traductionList);
    ...
    public function save_create_Menu($data){
    ...
    public function save_update_Menu($data){
        ...
        $e = MenuQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of Menu
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['Menu']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['Menu']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addMenu_child').bind('click.addMenu', function (){
                    $.post('"._SITE_URL."mod/act/MenuAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addMenu_child').bind('click.addMenu', function (){
                document.location= '"._SITE_URL."Menu/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'Menu', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'Menu','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formMenu #saveMenu').removeAttr('disabled');
                    $('#formMenu #saveMenu').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formMenu #saveMenu').css('cursor', 'default');
                    if($('#formMenu #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formMenu #saveMenu').bind('click.saveMenu', function (data){
                    $('#formMenu #saveMenu').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formMenu #saveMenu').css('cursor', 'progress');
                    $('#formMenu #saveMenu').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formMenu .tinymce').each(function(index) {
                        eval(' $(\"#formMenu #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formMenu select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formMenu\").find(\"[s='d']\").serializeArray();
                        $('#formMenu select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formMenu\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/MenuAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formMenu #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formMenu #formChangedMenu').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formMenu #saveMenu').unbind('click.saveMenu');
                $('#formMenu #saveMenu').remove();";
        }
        ...
        if($dataObj == null){
            ...
            $this->fields['Menu']['Title']['html'] = stdFieldRow(_("Titre"), input('text', 'Title',str_replace('"','&quot;',$dataObj->getTitle()), " otherTabs=1   placeholder='".str_replace("'","&#39;",_('Titre'))."' size='69'  v='TITLE' s='d' class=''")."", 'Title', "", @$this->commentsTitle, @$this->commentsTitle_css, '', ' ','no','');

            $this->fields['Menu']['Order']['html'] = stdFieldRow(_("Ordre du menu"), input('number', 'Order',$dataObj->getOrder(), " step='10' placeholder='".str_replace("'","&#39;",_('Ordre du menu'))."' v='ORDER' size='0' otherTabs=1 s='d' class='' otherTabs=1 "), 'Order', "", @$this->commentsOrder, @$this->commentsOrder_css, '', ' ','no','');

        $this->lockFormField(array(), $dataObj);
        if(!empty($this->setReadOnly) and $this->setReadOnly == 'all' ){ $this->lockFormField('all', $dataObj); }
        if(!isset($params['ChildHide'])  and !isset($this->Menu['request']['ChildHide']) ){
            
        }
        
        
        if($dataObj->getIdMenu()){
            $this->printLink =
                div(
                    ""
                    .@$this->CcPrintLinkDiv
                ...
                ,'MenuControls', "class='custom-controls'");
        ...
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            ...
            $this->add_button = 
                ...
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addMenu_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        
        if(empty($this->setReadOnly)){
            $this->formSaveBar = div(	div( input('button', 'saveMenu',$this->CcButtonName,' act="save" otherTabs="1" class="button-link-blue"')
                                .input('hidden', 'formChangedMenu','', 'j="formChanged"')
                                .input('hidden', 'idPk', urlencode($idPk), "s='d'")
                            .input('hidden', 'IdMenu', $dataObj->getIdMenu(), " otherTabs=1 s='d' pk").input('hidden', 'IdCreation',$dataObj->getIdCreation(), "   v='ID_CREATION' size='0' otherTabs=1 s='d' class='' otherTabs=1 ").input('hidden', 'IdModification',$dataObj->getIdModification(), "   v='ID_MODIFICATION' size='0' otherTabs=1 s='d' class='' otherTabs=1 ")
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'MenuControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formMenu');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['Menu']['tog']) and 
            $_SESSION['memoire']['onglet']['Menu']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['Menu']['Start']['html']
                
                .
$this->fields['Menu']['Title']['html']
.$this->fields['Menu']['Order']['html']
                
                .@$this->fields['Menu']['End']['html']
                .$this->formSaveBar
                .@$this->CcToInnerFormBottom
            ,"divCntMenu", "class='divStdform' CntTabs=1 ")
        , "id='formMenu' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
         
             
        if(!$this->siteTitle){ $this->siteTitle .=_("Formulaire Hiérarchie"); }
        # if not new, show child table
        if($dataObj->getIdMenu()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelMenu', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntMenuChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['Menu']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['Menu']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['Menu']['ixmemautocapp']);
        }
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntMenu');},500); 
    ".$toggleForm."
    bind_form('Menu','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        $this->fieldsRo['Menu']['Title']['html'] = stdFieldRow(_("Titre"), input('text','Title',$dataObj->getTitle()," readonly s='d'"), 'Title', "", @$this->commentsTitle, @$this->commentsTitle_css, 'readonly', ' ','no','');
$this->fieldsRo['Menu']['Order']['html'] = stdFieldRow(_("Ordre du menu"), input('text','Order',$dataObj->getOrder()," readonly s='d'"), 'Order', "", @$this->commentsOrder, @$this->commentsOrder_css, 'readonly', ' ','no','');


        if($fields == 'all'){
            foreach($this->fields['Menu'] as $field=>$ar){
                $this->fields['Menu'][$field]['html'] = $this->fieldsRo['Menu'][$field]['html'];
            }
    ...
    */
    ...
    public function getSelectJson($IdParent='', $selected=''){
        ...
        if($pcData->count()){
            ...
            $return = arrayToJson(array(array('0'=>'Aucun Hiérarchie')), $selected);
        }
