...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = WinnerQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Winner();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Winner';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_Winner($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Gagnants'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Gagnants'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_Winner($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['Winner']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Winner']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Winner']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Winner']['i'] = set_var($_SESSION['memoire']['onglet']['Winner']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Gagnants'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Winner';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdWinner'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new WinnerForm();
                                $e->Winner['request'] = $request;
                                $e->Winner['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new WinnerForm();
                $f->Winner['request'] = $request;
                if(!empty($request['ip'])){$f->Winner['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Winner']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Winner']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdWinner';
                    ...
                    unset($_SESSION['memoire']['onglet']['Winner']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = WinnerQuery::create()->findPk($request['i']);
                
            if($obj->countMonthWinners()){ $error = handleNotOkResponse(_("Cette entrée ne peut être supprimée car elle est présentement utilisée dans")." 'Mois'. ", '', true,'Gagnants'); die( $error['onReadyJs'] ); }
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Winner';
                if(!$obj->delete()){
        ...
        case 'getMonthWinnerList':
            if($_SESSION[_AUTH_VAR]->hasRights("WinnerMonthWinner",'r')){
                $listChild=true;

                $_SESSION['memoire']['onglet']['Winner']['child']['list'][$request['ip']]= 'MonthWinner';
                if(!empty($request['ip'])){$IdCurrentWinner = $request['ip'];}
                $IdSplit=""; 
                if(isset($request['ms']) and $request['ms'] == 'clear'){
                    $request['ms'] = '';$request['order'] = '';
                    unset($_SESSION['memoire']['search']['formMsWinnerMonthWinner'.$IdSplit]);
                    unset($_SESSION['memoire']['search']['formOrderWinnerMonthWinner']);
                    unset($_SESSION['memoire']['onglet']['WinnerMonthWinner']['pg']);
                    
                }
                if(!empty($request['order'])){
                    $search['order'] = json_decode($request['order'], true);
                    $_SESSION['memoire']['search']['formOrderWinnerMonthWinner'][$search['order']['col']] = $search['order']['sens'];
                    $search['order']= $_SESSION['memoire']['search']['formOrderWinnerMonthWinner'];
                }else if(!empty($_SESSION['memoire']['search']['formOrderWinnerMonthWinner'])){
                    $search['order']=$_SESSION['memoire']['search']['formOrderWinnerMonthWinner'];
                }
                if(!empty($request['ms'])){
                    parse_str($request['ms'], $search['ms']);
                    if($_SESSION['memoire']['search']['formMsWinnerMonthWinner'.$IdSplit] != $request['ms']){
                        unset($_SESSION['memoire']['onglet']['WinnerMonthWinner']['pg']);
                    }
                    $_SESSION['memoire']['search']['formMsWinnerMonthWinner'.$IdSplit] = $request['ms'];
                }else if(!empty($_SESSION['memoire']['search']['formMsWinnerMonthWinner'.$IdSplit])){
                    parse_str($_SESSION['memoire']['search']['formMsWinnerMonthWinner'.$IdSplit], $search['ms']);
                }
                if(!empty($request['pg'])){ $_SESSION['memoire']['onglet']['WinnerMonthWinner']['pg'] = $request['pg'];}
                
                $f = new WinnerForm();
                $f->MonthWinner['request'] = $request;
                if(!empty($request['ip'])){
                    $f->MonthWinner['parentId'] = $request['ip'];
                    $data['IdWinner'] = $request['ip'];
                }
                
                if(!empty($request['noHeader'])){$params['noHeader'] = $request['noHeader'];}
                $_SESSION['CurrentParent'] = 'Winner';
                $childTableData = $f->getMonthWinnerList(@$request['ip'],$request['pg'], @$request['ui'], @$request['pui'], true, @$search, @$params);
                
                unset($_SESSION['CurrentParent']);
                @$output['html'] .= div(@$f->CcMonthWinnerTopList.$childTableData['html'].@$f->CcMonthWinnerBottomList, 'MonthWinnerTableCntnr','class="child-wrapper"');
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Winner",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Winner",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Winner",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Winner",'r')){
            init_list_base($request,$search,'Winner','','','','');
            $e = new WinnerForm();
            $e->Winner['request'] = $request;
            if(!empty($request['ip'])){$e->Winner['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderWinner']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsWinner'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Winner($data, $request=array()){
        $e = new WinnerForm();
        $e->Winner['request'] = $request;
        return $e->save_update_Winner($data);
    }
    function save_create_Winner($data, $request=array()){
        $e = new WinnerForm();
        $e->Winner['request'] = $request;
        return $e->save_create_Winner($data);
    }
