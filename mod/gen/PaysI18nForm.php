...
###############################
/**
 * Form class on the 'PaysI18n' table.
 *
 */
class PaysI18nForm extends PaysI18n{
public $tableName="PaysI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('PaysI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."PaysI18n/edit/", "id='addPaysI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addPaysI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['PaysI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['PaysI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'PaysI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'PaysI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#PaysI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#PaysI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('PaysI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('PaysI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('PaysI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deletePaysI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='PaysI18nRow".$data->getPrimaryKey()."'
                    data-table='PaysI18n' 
            ...
            $tr .= input('hidden', 'rowCountPaysI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'PaysI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='PaysI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
        ...
        if(empty($hasParent)){
            $editEvent = "
        $(\"#PaysI18nListForm [j='deletePaysI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msPaysI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msPaysI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#PaysI18nListForm tr[ecf=1] td[j='editPaysI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."PaysI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."PaysI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#PaysI18nListForm [j='deletePaysI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msPaysI18nBt\').length>0){ $(\'#msPaysI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'PaysI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntPaysI18nListForm #addPaysI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'PaysI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#PaysI18nListForm tr[ecf=1] td[j='editPaysI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'PaysI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('PaysI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#PaysI18nListForm [j='deletePaysI18n']\").unbind('click');
        $('#PaysI18nListForm #addPaysI18n').unbind('click');
        $(\"#PaysI18nListForm tr[ecf=1] td[j='editPaysI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#PaysI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('PaysI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('PaysI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_PaysI18n($data){
    ...
    public function save_update_PaysI18n($data){

        
        $e = PaysI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of PaysI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['PaysI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['PaysI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addPaysI18n_child').bind('click.addPaysI18n', function (){
                    $.post('"._SITE_URL."mod/act/PaysI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addPaysI18n_child').bind('click.addPaysI18n', function (){
                document.location= '"._SITE_URL."PaysI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'PaysI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'PaysI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formPaysI18n #savePaysI18n').removeAttr('disabled');
                    $('#formPaysI18n #savePaysI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formPaysI18n #savePaysI18n').css('cursor', 'default');
                    if($('#formPaysI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formPaysI18n #savePaysI18n').bind('click.savePaysI18n', function (data){
                    $('#formPaysI18n #savePaysI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formPaysI18n #savePaysI18n').css('cursor', 'progress');
                    $('#formPaysI18n #savePaysI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formPaysI18n .tinymce').each(function(index) {
                        eval(' $(\"#formPaysI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formPaysI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formPaysI18n\").find(\"[s='d']\").serializeArray();
                        $('#formPaysI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formPaysI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/PaysI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formPaysI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formPaysI18n #formChangedPaysI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formPaysI18n #savePaysI18n').unbind('click.savePaysI18n');
                $('#formPaysI18n #savePaysI18n').remove();";
        }
        
        if($dataObj == null){
            $this->PaysI18n['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new PaysI18n();
            $this->PaysI18n['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->PaysI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdPays()){
            $this->printLink =
                ...
                ,'PaysI18nControls', "class='custom-controls'");
        ...
        $this->formTitle = p(href("",_SITE_URL.'PaysI18n').$titre_form_str, 'class="breadcrumb"'); 
        $this->add_button= '';
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            
            $this->add_button = 
                
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addPaysI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'PaysI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formPaysI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['PaysI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['PaysI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['PaysI18n']['Start']['html']
                
                
                
                .@$this->fields['PaysI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntPaysI18n", "class='divStdform' CntTabs=1 ")
        , "id='formPaysI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdPays()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelPaysI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntPaysI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['PaysI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['PaysI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['PaysI18n']['ixmemautocapp']);
        }
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntPaysI18n');},500); 
    ".$toggleForm."
    bind_form('PaysI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['PaysI18n'] as $field=>$ar){
                $this->fields['PaysI18n'][$field]['html'] = $this->fieldsRo['PaysI18n'][$field]['html'];
            }
    ...
    */
    ...
    $this->ajaxPageAct = 'PaysI18nAct.php';
