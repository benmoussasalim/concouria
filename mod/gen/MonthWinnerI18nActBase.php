...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = MonthWinnerI18nQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new MonthWinnerI18n();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'MonthWinnerI18n';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_MonthWinnerI18n($data, $request);
                    ...
                    $e = save_create_MonthWinnerI18n($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['MonthWinnerI18n']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['MonthWinnerI18n']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['MonthWinnerI18n']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['MonthWinnerI18n']['i'] = set_var($_SESSION['memoire']['onglet']['MonthWinnerI18n']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'MonthWinnerI18n';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdMonthWinner'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new MonthWinnerI18nForm();
                                $e->MonthWinnerI18n['request'] = $request;
                                $e->MonthWinnerI18n['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new MonthWinnerI18nForm();
                $f->MonthWinnerI18n['request'] = $request;
                if(!empty($request['ip'])){$f->MonthWinnerI18n['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['MonthWinnerI18n']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['MonthWinnerI18n']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    ...
                    unset($_SESSION['memoire']['onglet']['MonthWinnerI18n']['ixmemautoc']);
                }
                ...
                $IdCurrentMonthWinnerI18n = $data['i'];
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = MonthWinnerI18nQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'MonthWinnerI18n';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."MonthWinnerI18n",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."MonthWinnerI18n",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."MonthWinnerI18n",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("MonthWinnerI18n",'r')){
            init_list_base($request,$search,'MonthWinnerI18n','','','','');
            $e = new MonthWinnerI18nForm();
            $e->MonthWinnerI18n['request'] = $request;
            if(!empty($request['ip'])){$e->MonthWinnerI18n['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderMonthWinnerI18n']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsMonthWinnerI18n'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_MonthWinnerI18n($data, $request=array()){
        $e = new MonthWinnerI18nForm();
        $e->MonthWinnerI18n['request'] = $request;
        return $e->save_update_MonthWinnerI18n($data);
    }
    function save_create_MonthWinnerI18n($data, $request=array()){
        $e = new MonthWinnerI18nForm();
        $e->MonthWinnerI18n['request'] = $request;
        return $e->save_create_MonthWinnerI18n($data);
    }
