...
###############################
/**
 * Form class on the 'StatistiqueI18n' table.
 *
 */
class StatistiqueI18nForm extends StatistiqueI18n{
public $tableName="StatistiqueI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('StatistiqueI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."StatistiqueI18n/edit/", "id='addStatistiqueI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addStatistiqueI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['StatistiqueI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['StatistiqueI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'StatistiqueI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'StatistiqueI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#StatistiqueI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#StatistiqueI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('StatistiqueI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('StatistiqueI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('StatistiqueI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteStatistiqueI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='StatistiqueI18nRow".$data->getPrimaryKey()."'
                    data-table='StatistiqueI18n' 
            ...
            $tr .= input('hidden', 'rowCountStatistiqueI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'StatistiqueI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='StatistiqueI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
                ,'listForm',' class="ac-list contentHolderListP" ')
                .$this->CcToListBottom
                .@$bottomRow
            , 'StatistiqueI18nListForm');
        
        $hasParent = json_decode($IdParent);
        if(empty($hasParent)){
            $editEvent = "
        $(\"#StatistiqueI18nListForm [j='deleteStatistiqueI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msStatistiqueI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msStatistiqueI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#StatistiqueI18nListForm tr[ecf=1] td[j='editStatistiqueI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."StatistiqueI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."StatistiqueI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#StatistiqueI18nListForm [j='deleteStatistiqueI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msStatistiqueI18nBt\').length>0){ $(\'#msStatistiqueI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'StatistiqueI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntStatistiqueI18nListForm #addStatistiqueI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'StatistiqueI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#StatistiqueI18nListForm tr[ecf=1] td[j='editStatistiqueI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'StatistiqueI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('StatistiqueI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#StatistiqueI18nListForm [j='deleteStatistiqueI18n']\").unbind('click');
        $('#StatistiqueI18nListForm #addStatistiqueI18n').unbind('click');
        $(\"#StatistiqueI18nListForm tr[ecf=1] td[j='editStatistiqueI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#StatistiqueI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('StatistiqueI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('StatistiqueI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_StatistiqueI18n($data){

        unset($data['IdStatistique']);
        $e = new StatistiqueI18n();
    ...
    public function save_update_StatistiqueI18n($data){

        
        $e = StatistiqueI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of StatistiqueI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['StatistiqueI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['StatistiqueI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addStatistiqueI18n_child').bind('click.addStatistiqueI18n', function (){
                    $.post('"._SITE_URL."mod/act/StatistiqueI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addStatistiqueI18n_child').bind('click.addStatistiqueI18n', function (){
                document.location= '"._SITE_URL."StatistiqueI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'StatistiqueI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'StatistiqueI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formStatistiqueI18n #saveStatistiqueI18n').removeAttr('disabled');
                    $('#formStatistiqueI18n #saveStatistiqueI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formStatistiqueI18n #saveStatistiqueI18n').css('cursor', 'default');
                    if($('#formStatistiqueI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formStatistiqueI18n #saveStatistiqueI18n').bind('click.saveStatistiqueI18n', function (data){
                    $('#formStatistiqueI18n #saveStatistiqueI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formStatistiqueI18n #saveStatistiqueI18n').css('cursor', 'progress');
                    $('#formStatistiqueI18n #saveStatistiqueI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formStatistiqueI18n .tinymce').each(function(index) {
                        eval(' $(\"#formStatistiqueI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formStatistiqueI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formStatistiqueI18n\").find(\"[s='d']\").serializeArray();
                        $('#formStatistiqueI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formStatistiqueI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/StatistiqueI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formStatistiqueI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formStatistiqueI18n #formChangedStatistiqueI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formStatistiqueI18n #saveStatistiqueI18n').unbind('click.saveStatistiqueI18n');
                $('#formStatistiqueI18n #saveStatistiqueI18n').remove();";
        }
        
        if($dataObj == null){
            $this->StatistiqueI18n['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new StatistiqueI18n();
            $this->StatistiqueI18n['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->StatistiqueI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdStatistique()){
            $this->printLink =
                ...
                ,'StatistiqueI18nControls', "class='custom-controls'");
        ...
        $this->formTitle = p(href("",_SITE_URL.'StatistiqueI18n').$titre_form_str, 'class="breadcrumb"'); 
        $this->add_button= '';
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            
            $this->add_button = 
                
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addStatistiqueI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'StatistiqueI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formStatistiqueI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['StatistiqueI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['StatistiqueI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['StatistiqueI18n']['Start']['html']
                
                
                
                .@$this->fields['StatistiqueI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntStatistiqueI18n", "class='divStdform' CntTabs=1 ")
        , "id='formStatistiqueI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdStatistique()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelStatistiqueI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntStatistiqueI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['StatistiqueI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['StatistiqueI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['StatistiqueI18n']['ixmemautocapp']);
        }
    ...
    ".$jqueryDatePicker."
    bind_ui_active('StatistiqueI18n');
    ".$this->AddButtonJs."
    ".$this->SaveButtonJs."
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntStatistiqueI18n');},500); 
    ".$toggleForm."
    bind_form('StatistiqueI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['StatistiqueI18n'] as $field=>$ar){
                $this->fields['StatistiqueI18n'][$field]['html'] = $this->fieldsRo['StatistiqueI18n'][$field]['html'];
            }
    ...
    */
    ...
    $this->ajaxPageAct = 'StatistiqueI18nAct.php';
