...
###############################
/**
 * Form class on the 'BlockI18n' table.
 *
 */
class BlockI18nForm extends BlockI18n{
public $tableName="BlockI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('BlockI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."BlockI18n/edit/", "id='addBlockI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addBlockI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['BlockI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['BlockI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'BlockI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'BlockI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#BlockI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#BlockI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('BlockI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('BlockI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('BlockI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteBlockI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='BlockI18nRow".$data->getPrimaryKey()."'
                    data-table='BlockI18n' 
            ...
            $tr .= input('hidden', 'rowCountBlockI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'BlockI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='BlockI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
        ...
        if(empty($hasParent)){
            $editEvent = "
        $(\"#BlockI18nListForm [j='deleteBlockI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msBlockI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msBlockI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#BlockI18nListForm tr[ecf=1] td[j='editBlockI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."BlockI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."BlockI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#BlockI18nListForm [j='deleteBlockI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msBlockI18nBt\').length>0){ $(\'#msBlockI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'BlockI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntBlockI18nListForm #addBlockI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'BlockI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#BlockI18nListForm tr[ecf=1] td[j='editBlockI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'BlockI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('BlockI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#BlockI18nListForm [j='deleteBlockI18n']\").unbind('click');
        $('#BlockI18nListForm #addBlockI18n').unbind('click');
        $(\"#BlockI18nListForm tr[ecf=1] td[j='editBlockI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#BlockI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('BlockI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('BlockI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_BlockI18n($data){
    ...
    public function save_update_BlockI18n($data){

        
        $e = BlockI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of BlockI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['BlockI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['BlockI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addBlockI18n_child').bind('click.addBlockI18n', function (){
                    $.post('"._SITE_URL."mod/act/BlockI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addBlockI18n_child').bind('click.addBlockI18n', function (){
                document.location= '"._SITE_URL."BlockI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'BlockI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'BlockI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formBlockI18n #saveBlockI18n').removeAttr('disabled');
                    $('#formBlockI18n #saveBlockI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formBlockI18n #saveBlockI18n').css('cursor', 'default');
                    if($('#formBlockI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formBlockI18n #saveBlockI18n').bind('click.saveBlockI18n', function (data){
                    $('#formBlockI18n #saveBlockI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formBlockI18n #saveBlockI18n').css('cursor', 'progress');
                    $('#formBlockI18n #saveBlockI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formBlockI18n .tinymce').each(function(index) {
                        eval(' $(\"#formBlockI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formBlockI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formBlockI18n\").find(\"[s='d']\").serializeArray();
                        $('#formBlockI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formBlockI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/BlockI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formBlockI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formBlockI18n #formChangedBlockI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formBlockI18n #saveBlockI18n').unbind('click.saveBlockI18n');
                $('#formBlockI18n #saveBlockI18n').remove();";
        }
        
        if($dataObj == null){
            $this->BlockI18n['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new BlockI18n();
            $this->BlockI18n['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->BlockI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdBlock()){
            $this->printLink =
                ...
                ,'BlockI18nControls', "class='custom-controls'");
        ...
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            ...
            $this->add_button = 
                ...
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addBlockI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'BlockI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formBlockI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['BlockI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['BlockI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['BlockI18n']['Start']['html']
                
                
                
                .@$this->fields['BlockI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntBlockI18n", "class='divStdform' CntTabs=1 ")
        , "id='formBlockI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdBlock()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelBlockI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntBlockI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['BlockI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['BlockI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['BlockI18n']['ixmemautocapp']);
        }
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntBlockI18n');},500); 
    ".$toggleForm."
    bind_form('BlockI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['BlockI18n'] as $field=>$ar){
                $this->fields['BlockI18n'][$field]['html'] = $this->fieldsRo['BlockI18n'][$field]['html'];
            }
    ...
    */
    ...
    $this->ajaxPageAct = 'BlockI18nAct.php';
