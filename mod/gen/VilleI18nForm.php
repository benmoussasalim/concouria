...
###############################
/**
 * Form class on the 'VilleI18n' table.
 *
 */
class VilleI18nForm extends VilleI18n{
public $tableName="VilleI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('VilleI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."VilleI18n/edit/", "id='addVilleI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addVilleI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['VilleI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['VilleI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'VilleI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'VilleI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#VilleI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#VilleI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('VilleI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('VilleI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('VilleI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteVilleI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='VilleI18nRow".$data->getPrimaryKey()."'
                    data-table='VilleI18n' 
            ...
            $tr .= input('hidden', 'rowCountVilleI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'VilleI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='VilleI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
        ...
        if(empty($hasParent)){
            $editEvent = "
        $(\"#VilleI18nListForm [j='deleteVilleI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msVilleI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msVilleI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#VilleI18nListForm tr[ecf=1] td[j='editVilleI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."VilleI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."VilleI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#VilleI18nListForm [j='deleteVilleI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msVilleI18nBt\').length>0){ $(\'#msVilleI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'VilleI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntVilleI18nListForm #addVilleI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'VilleI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#VilleI18nListForm tr[ecf=1] td[j='editVilleI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'VilleI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('VilleI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#VilleI18nListForm [j='deleteVilleI18n']\").unbind('click');
        $('#VilleI18nListForm #addVilleI18n').unbind('click');
        $(\"#VilleI18nListForm tr[ecf=1] td[j='editVilleI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#VilleI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('VilleI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('VilleI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_VilleI18n($data){
    ...
    public function save_update_VilleI18n($data){

        
        $e = VilleI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of VilleI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['VilleI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['VilleI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addVilleI18n_child').bind('click.addVilleI18n', function (){
                    $.post('"._SITE_URL."mod/act/VilleI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addVilleI18n_child').bind('click.addVilleI18n', function (){
                document.location= '"._SITE_URL."VilleI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'VilleI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'VilleI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formVilleI18n #saveVilleI18n').removeAttr('disabled');
                    $('#formVilleI18n #saveVilleI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formVilleI18n #saveVilleI18n').css('cursor', 'default');
                    if($('#formVilleI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formVilleI18n #saveVilleI18n').bind('click.saveVilleI18n', function (data){
                    $('#formVilleI18n #saveVilleI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formVilleI18n #saveVilleI18n').css('cursor', 'progress');
                    $('#formVilleI18n #saveVilleI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formVilleI18n .tinymce').each(function(index) {
                        eval(' $(\"#formVilleI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formVilleI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formVilleI18n\").find(\"[s='d']\").serializeArray();
                        $('#formVilleI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formVilleI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/VilleI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formVilleI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formVilleI18n #formChangedVilleI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formVilleI18n #saveVilleI18n').unbind('click.saveVilleI18n');
                $('#formVilleI18n #saveVilleI18n').remove();";
        }
        
        if($dataObj == null){
            $this->VilleI18n['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new VilleI18n();
            $this->VilleI18n['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->VilleI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdVille()){
            $this->printLink =
                ...
                ,'VilleI18nControls', "class='custom-controls'");
        ...
        $this->formTitle = p(href("",_SITE_URL.'VilleI18n').$titre_form_str, 'class="breadcrumb"'); 
        $this->add_button= '';
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            
            $this->add_button = 
                
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addVilleI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'VilleI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formVilleI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['VilleI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['VilleI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['VilleI18n']['Start']['html']
                
                
                
                .@$this->fields['VilleI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntVilleI18n", "class='divStdform' CntTabs=1 ")
        , "id='formVilleI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdVille()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelVilleI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntVilleI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['VilleI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['VilleI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['VilleI18n']['ixmemautocapp']);
        }
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntVilleI18n');},500); 
    ".$toggleForm."
    bind_form('VilleI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['VilleI18n'] as $field=>$ar){
                $this->fields['VilleI18n'][$field]['html'] = $this->fieldsRo['VilleI18n'][$field]['html'];
            }
    ...
    */
    ...
    $this->ajaxPageAct = 'VilleI18nAct.php';
