...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = GroupRightQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new GroupRight();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        case 'bulkUpdateFormGroupRightAuthy':
            if($_SESSION[_AUTH_VAR]->hasRights("GroupRightGroupRightAuthy",'r')){
                $f = new GroupRightForm();
                $f->GroupRight['request'] = $request;
                $f->GroupRight['parentId'] = $request['ip'];
                parse_str($request['i'], $data);
                $data['i'] = json_encode($data);
                $data['ip'] = $request['ip'];
                $output = $f->bulkUpdateFormGroupRightAuthy($data,$request['ui']);
            }
        break;
        case 'BUsaveGroupRightAuthy':
            $_SESSION['CurrentRights']="GroupRightGroupRightAuthy";
            parse_str($request['d'], $data);
            if($data['idPk'] && $request['c'] == 'y'){
                parse_str($data['idPk'], $data['ids']);
            }elseif($data['idPk']){
                $data['ids'] = json_decode(urldecode($data['idPk']), true);
            }
            if(is_array($data['ids'])){
                foreach($data['ids'] as $id){
                    $data['i'] = json_decode($id, true);
                    if($data['i']){
                        
                        
                        
                        $e = GroupRightAuthyQuery::create()->findPk($data['i']);
                        if(!$e){ $e = new GroupRightAuthy();$e->setPrimaryKey($data['i']);}
                        
                    if($data['ck_Primary'] == 'up'){
                        if($data['Primary']==''){$data['Primary'] = NULL;}
                        $e->setPrimary($data['Primary']);
                    }
                    if($data['ck_IsSet'] == 'up'){
                        if($data['IsSet']==''){$data['IsSet'] = NULL;}
                        $e->setIsSet($data['IsSet']);
                    }
                ...
                $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Groupe'));
                $updated[] = $e->getPrimaryKey();
                    
                        }else{
                            $error = handleValidationError($e, $request['ui'],_('Groupe'), $extValidationErr);
                            $previousData = $data;
                        }
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'GroupRight';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_GroupRight($data, $request);
            ...
            fopen(_INSTALL_PATH.'mod/tmp/'.$IdPrimaryKey.'.authy','w');
                    ...
                    $e = save_create_GroupRight($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['GroupRight']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['GroupRight']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['GroupRight']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['GroupRight']['i'] = set_var($_SESSION['memoire']['onglet']['GroupRight']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Groupe'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'GroupRight';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdGroupRight'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new GroupRightForm();
                                $e->GroupRight['request'] = $request;
                                $e->GroupRight['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new GroupRightForm();
                $f->GroupRight['request'] = $request;
                if(!empty($request['ip'])){$f->GroupRight['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['GroupRight']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['GroupRight']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdGroupRight';
                    ...
                    unset($_SESSION['memoire']['onglet']['GroupRight']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = GroupRightQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'GroupRight';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."GroupRight",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."GroupRight",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."GroupRight",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("GroupRight",'r')){
            init_list_base($request,$search,'GroupRight','','','','');
            $e = new GroupRightForm();
            $e->GroupRight['request'] = $request;
            if(!empty($request['ip'])){$e->GroupRight['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderGroupRight']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsGroupRight'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_GroupRight($data, $request=array()){
        $e = new GroupRightForm();
        $e->GroupRight['request'] = $request;
        return $e->save_update_GroupRight($data);
    }
    function save_create_GroupRight($data, $request=array()){
        $e = new GroupRightForm();
        $e->GroupRight['request'] = $request;
        return $e->save_create_GroupRight($data);
    }
