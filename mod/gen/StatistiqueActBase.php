...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = StatistiqueQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Statistique();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Statistique';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_Statistique($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        try{
                            $e->setLocale('fr_CA');
                            if(isset($data['StatistiqueI18n_Name_frCA'])){$e->setName($data['StatistiqueI18n_Name_frCA']);}
                            if(isset($data['StatistiqueI18n_Text_frCA'])){$e->setText($data['StatistiqueI18n_Text_frCA']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new StatistiqueI18n();
                            $mt->setLocale('fr_CA');
                            
                            if(isset($data['StatistiqueI18n_Name_frCA'])){$mt->setName($data['StatistiqueI18n_Name_frCA']);}
                            if(isset($data['StatistiqueI18n_Text_frCA'])){$mt->setText($data['StatistiqueI18n_Text_frCA']);}
                            $e->addStatistiqueI18n($mt);$e->save();
                        }
                    
                        try{
                            $e->setLocale('en_US');
                            if(isset($data['StatistiqueI18n_Name_enUS'])){$e->setName($data['StatistiqueI18n_Name_enUS']);}
                            if(isset($data['StatistiqueI18n_Text_enUS'])){$e->setText($data['StatistiqueI18n_Text_enUS']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new StatistiqueI18n();
                            $mt->setLocale('en_US');
                            
                            if(isset($data['StatistiqueI18n_Name_enUS'])){$mt->setName($data['StatistiqueI18n_Name_enUS']);}
                            if(isset($data['StatistiqueI18n_Text_enUS'])){$mt->setText($data['StatistiqueI18n_Text_enUS']);}
                            $e->addStatistiqueI18n($mt);$e->save();
                        }
                    
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Statistiques'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Statistiques'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_Statistique($data, $request);
                    
                    
                        
                        
                        
                    $e->setLocale('fr_CA');$e->setName($data['StatistiqueI18n_Name_frCA']);
                    $e->setLocale('fr_CA');$e->setText($data['StatistiqueI18n_Text_frCA']);
                    $e->setLocale('en_US');$e->setName($data['StatistiqueI18n_Name_enUS']);
                    $e->setLocale('en_US');$e->setText($data['StatistiqueI18n_Text_enUS']);
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        $e->save();
                        
                        unset($_SESSION['memoire']['onglet']['Statistique']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Statistique']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Statistique']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Statistique']['i'] = set_var($_SESSION['memoire']['onglet']['Statistique']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Statistiques'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Statistique';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdStatistique'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new StatistiqueForm();
                                $e->Statistique['request'] = $request;
                                $e->Statistique['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new StatistiqueForm();
                $f->Statistique['request'] = $request;
                if(!empty($request['ip'])){$f->Statistique['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Statistique']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Statistique']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdStatistique';
                    ...
                    unset($_SESSION['memoire']['onglet']['Statistique']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = StatistiqueQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Statistique';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Statistique",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Statistique",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Statistique",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Statistique",'r')){
            init_list_base($request,$search,'Statistique','','','','');
            $e = new StatistiqueForm();
            $e->Statistique['request'] = $request;
            if(!empty($request['ip'])){$e->Statistique['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderStatistique']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsStatistique'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Statistique($data, $request=array()){
        $e = new StatistiqueForm();
        $e->Statistique['request'] = $request;
        return $e->save_update_Statistique($data);
    }
    function save_create_Statistique($data, $request=array()){
        $e = new StatistiqueForm();
        $e->Statistique['request'] = $request;
        return $e->save_create_Statistique($data);
    }
