...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = ProvinceQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Province();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Province';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_Province($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        try{
                            $e->setLocale('fr_CA');
                            if(isset($data['ProvinceI18n_Name_frCA'])){$e->setName($data['ProvinceI18n_Name_frCA']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new ProvinceI18n();
                            $mt->setLocale('fr_CA');
                            
                            if(isset($data['ProvinceI18n_Name_frCA'])){$mt->setName($data['ProvinceI18n_Name_frCA']);}
                            $e->addProvinceI18n($mt);$e->save();
                        }
                    
                        try{
                            $e->setLocale('en_US');
                            if(isset($data['ProvinceI18n_Name_enUS'])){$e->setName($data['ProvinceI18n_Name_enUS']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new ProvinceI18n();
                            $mt->setLocale('en_US');
                            
                            if(isset($data['ProvinceI18n_Name_enUS'])){$mt->setName($data['ProvinceI18n_Name_enUS']);}
                            $e->addProvinceI18n($mt);$e->save();
                        }
                    
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Province'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Province'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_Province($data, $request);
                    
                    
                        
                        
                        
                    $e->setLocale('fr_CA');$e->setName($data['ProvinceI18n_Name_frCA']);
                    $e->setLocale('en_US');$e->setName($data['ProvinceI18n_Name_enUS']);
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        $e->save();
                        
                        unset($_SESSION['memoire']['onglet']['Province']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Province']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Province']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Province']['i'] = set_var($_SESSION['memoire']['onglet']['Province']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Province'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Province';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdProvince'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new ProvinceForm();
                                $e->Province['request'] = $request;
                                $e->Province['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new ProvinceForm();
                $f->Province['request'] = $request;
                if(!empty($request['ip'])){$f->Province['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Province']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Province']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdProvince';
                    ...
                    unset($_SESSION['memoire']['onglet']['Province']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = ProvinceQuery::create()->findPk($request['i']);
                
            if($obj->countAccounts()){ $error = handleNotOkResponse(_("Cette entrée ne peut être supprimée car elle est présentement utilisée dans")." 'Compte'. ", '', true,'Province'); die( $error['onReadyJs'] ); }
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Province';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Province",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Province",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Province",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Province",'r')){
            init_list_base($request,$search,'Province','','','','');
            $e = new ProvinceForm();
            $e->Province['request'] = $request;
            if(!empty($request['ip'])){$e->Province['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderProvince']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsProvince'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Province($data, $request=array()){
        $e = new ProvinceForm();
        $e->Province['request'] = $request;
        return $e->save_update_Province($data);
    }
    function save_create_Province($data, $request=array()){
        $e = new ProvinceForm();
        $e->Province['request'] = $request;
        return $e->save_create_Province($data);
    }
