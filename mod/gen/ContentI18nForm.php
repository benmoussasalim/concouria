...
###############################
/**
 * Form class on the 'ContentI18n' table.
 *
 */
class ContentI18nForm extends ContentI18n{
public $tableName="ContentI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('ContentI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."ContentI18n/edit/", "id='addContentI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addContentI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['ContentI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['ContentI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'ContentI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'ContentI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#ContentI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#ContentI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('ContentI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('ContentI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('ContentI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteContentI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='ContentI18nRow".$data->getPrimaryKey()."'
                    data-table='ContentI18n' 
            ...
            $tr .= input('hidden', 'rowCountContentI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'ContentI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='ContentI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
        ...
        if(empty($hasParent)){
            $editEvent = "
        $(\"#ContentI18nListForm [j='deleteContentI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msContentI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msContentI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#ContentI18nListForm tr[ecf=1] td[j='editContentI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."ContentI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."ContentI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#ContentI18nListForm [j='deleteContentI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msContentI18nBt\').length>0){ $(\'#msContentI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'ContentI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntContentI18nListForm #addContentI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'ContentI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#ContentI18nListForm tr[ecf=1] td[j='editContentI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'ContentI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('ContentI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#ContentI18nListForm [j='deleteContentI18n']\").unbind('click');
        $('#ContentI18nListForm #addContentI18n').unbind('click');
        $(\"#ContentI18nListForm tr[ecf=1] td[j='editContentI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#ContentI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('ContentI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('ContentI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_ContentI18n($data){

        unset($data['IdContent']);
        $e = new ContentI18n();
        
        
        if(isset($data['Version'])){$e->setVersion(($data['Version']=='')?null:$data['Version']);}
        ...
        $e->setVersion(($data['Version']=='')?null:$data['Version']);
    ...
    public function save_update_ContentI18n($data){

        
        $e = ContentI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of ContentI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['ContentI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['ContentI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addContentI18n_child').bind('click.addContentI18n', function (){
                    $.post('"._SITE_URL."mod/act/ContentI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addContentI18n_child').bind('click.addContentI18n', function (){
                document.location= '"._SITE_URL."ContentI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'ContentI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'ContentI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formContentI18n #saveContentI18n').removeAttr('disabled');
                    $('#formContentI18n #saveContentI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formContentI18n #saveContentI18n').css('cursor', 'default');
                    if($('#formContentI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formContentI18n #saveContentI18n').bind('click.saveContentI18n', function (data){
                    $('#formContentI18n #saveContentI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formContentI18n #saveContentI18n').css('cursor', 'progress');
                    $('#formContentI18n #saveContentI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formContentI18n .tinymce').each(function(index) {
                        eval(' $(\"#formContentI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formContentI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formContentI18n\").find(\"[s='d']\").serializeArray();
                        $('#formContentI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formContentI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/ContentI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formContentI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formContentI18n #formChangedContentI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formContentI18n #saveContentI18n').unbind('click.saveContentI18n');
                $('#formContentI18n #saveContentI18n').remove();";
        }
        
        if($dataObj == null){
            $this->ContentI18n['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new ContentI18n();
            $this->ContentI18n['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->ContentI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdContent()){
            $this->printLink =
                ...
                ,'ContentI18nControls', "class='custom-controls'");
        ...
        $this->formTitle = p(href("",_SITE_URL.'ContentI18n').$titre_form_str, 'class="breadcrumb"'); 
        $this->add_button= '';
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            
            $this->add_button = 
                
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addContentI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'ContentI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formContentI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['ContentI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['ContentI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['ContentI18n']['Start']['html']
                
                
                
                .@$this->fields['ContentI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntContentI18n", "class='divStdform' CntTabs=1 ")
        , "id='formContentI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdContent()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelContentI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntContentI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['ContentI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['ContentI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['ContentI18n']['ixmemautocapp']);
        }
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntContentI18n');},500); 
    ".$toggleForm."
    bind_form('ContentI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['ContentI18n'] as $field=>$ar){
                $this->fields['ContentI18n'][$field]['html'] = $this->fieldsRo['ContentI18n'][$field]['html'];
            }
    ...
    */
    ...
    $this->ajaxPageAct = 'ContentI18nAct.php';
