    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = SliderFileQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new SliderFile();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        case 'ForceDl':
            if($_SESSION[_AUTH_VAR]->hasRights('SliderFile','r')
            || $_SESSION[_AUTH_VAR]->hasRights($request['pc']."SliderFile",'r')
            ){
                $dataObj= SliderFileQuery::create()->findPk($request['i']);
...
array('name'=>$_FILES["file"]["name"][$f],'size'=>$_FILES["file"]["size"][$f],'type'=>$_FILES["file"]["type"][$f],'tmp_name'=>$_FILES["file"]["tmp_name"][$f],'error'=>$_FILES["file"]["error"][$f]);
                    ...
                    if( $path_info["extension"] ){
                        ...
                        if($tabIp){
                            foreach(array_unique($tabIp) as $ip){
                                $e = save_create_SliderFile($data, $request);
                                $e->setIdSlider($ip);
                                $path_rep_fichier = 'mod/file/';
                                $path_fichier = 'mod/file/SliderFile/';
                                ...
                                if(@$data['error'] == ''){
                                    if($_GET['blob']==1){
                                        ...
                                        if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'SliderFile';}
                                        $e->fromArray($data);
                                        if ($e->validate()){
                                            $e->save();
            ...
            case 'save':
                ...
                if(!empty($pkPresent)){
                    ...
                    ## Save
                     ...
                     $e = save_update_SliderFile($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['SliderFile']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['SliderFile']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['SliderFile']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['SliderFile']['i'] = set_var($_SESSION['memoire']['onglet']['SliderFile']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);

                        }
                        
                        
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $data['IdSlider'] = json_encode($e->getIdSlider());
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'SliderFile';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        ...
                        if(!$data['IdSlider'] && !$request['ip'] or ($request['je'] =='SliderFileTable' and !$request['ip']) ){
                            $output['onReadyJs'] .= "$('#formMsSliderFile #msSliderFileBt').click();".$closeDiag."";
                        }else{
                            $output['onReadyJs'] .= "$('.child_pannel [p=\"SliderFile\"]').click();".$closeDiag."";
                        }
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new SliderFileForm();
                                $e->SliderFile['request'] = $request;
                                $e->SliderFile['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new SliderFileForm();
                $f->SliderFile['request'] = $request;
                if(!empty($request['ip'])){$f->SliderFile['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['SliderFile']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['SliderFile']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdSliderFile';
                    ...
                    unset($_SESSION['memoire']['onglet']['SliderFile']['ixmemautoc']);
                }
                ...
                $IdCurrentSliderFile = $data['i'];
                
            $q = SliderFileQuery::create()->filterByIdSliderFile($IdCurrentSliderFile)->setFormatter(ModelCriteria::FORMAT_ARRAY)->findOne();

            @$output['onReadyJs'] .= "
                $('#formSliderFile .divtr.readonly .divtd[in=\"inSize\"] label.readonly').text('".addslashes($q['Size'])."');
                $('#formSliderFile #Size').val('".addslashes($q['Size'])."');
                $('#formSliderFile .divtr.readonly .divtd[in=\"inName\"] label.readonly').text('".addslashes($q['Name'])."');
                $('#formSliderFile #Name').val('".addslashes($q['Name'])."');
            ";
            ...
            case 'delete':
                ...
                $obj = SliderFileQuery::create()->findPk($request['i']);
        ...
        if(is_file(_INSTALL_PATH.$obj->getFichier())){
            unlink(_INSTALL_PATH.$obj->getFichier());
                ...
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'SliderFile';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."SliderFile",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."SliderFile",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."SliderFile",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("SliderFile",'r')){
            init_list_base($request,$search,'SliderFile','','','','');
            $e = new SliderFileForm();
            $e->SliderFile['request'] = $request;
            if(!empty($request['ip'])){$e->SliderFile['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderSliderFile']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsSliderFile'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_SliderFile($data, $request=array()){
        $e = new SliderFileForm();
        $e->SliderFile['request'] = $request;
        return $e->save_update_SliderFile($data);
    }
    function save_create_SliderFile($data, $request=array()){
        $e = new SliderFileForm();
        $e->SliderFile['request'] = $request;
        return $e->save_create_SliderFile($data);
    }
