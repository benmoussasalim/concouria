...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = TicketQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Ticket();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Ticket';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_Ticket($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Ticket'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Ticket'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_Ticket($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['Ticket']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Ticket']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Ticket']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Ticket']['i'] = set_var($_SESSION['memoire']['onglet']['Ticket']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Ticket'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Ticket';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdTicket'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new TicketForm();
                                $e->Ticket['request'] = $request;
                                $e->Ticket['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new TicketForm();
                $f->Ticket['request'] = $request;
                if(!empty($request['ip'])){$f->Ticket['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Ticket']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Ticket']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdTicket';
                    ...
                    unset($_SESSION['memoire']['onglet']['Ticket']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = TicketQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Ticket';
                if(!$obj->delete()){
        ...
        case 'getTicketChatList':
            if($_SESSION[_AUTH_VAR]->hasRights("TicketTicketChat",'r')){
                $listChild=true;

                $_SESSION['memoire']['onglet']['Ticket']['child']['list'][$request['ip']]= 'TicketChat';
                if(!empty($request['ip'])){$IdCurrentTicket = $request['ip'];}
                $IdSplit=""; 
                if(isset($request['ms']) and $request['ms'] == 'clear'){
                    $request['ms'] = '';$request['order'] = '';
                    unset($_SESSION['memoire']['search']['formMsTicketTicketChat'.$IdSplit]);
                    unset($_SESSION['memoire']['search']['formOrderTicketTicketChat']);
                    unset($_SESSION['memoire']['onglet']['TicketTicketChat']['pg']);
                    
                }
                if(!empty($request['order'])){
                    $search['order'] = json_decode($request['order'], true);
                    $_SESSION['memoire']['search']['formOrderTicketTicketChat'][$search['order']['col']] = $search['order']['sens'];
                    $search['order']= $_SESSION['memoire']['search']['formOrderTicketTicketChat'];
                }else if(!empty($_SESSION['memoire']['search']['formOrderTicketTicketChat'])){
                    $search['order']=$_SESSION['memoire']['search']['formOrderTicketTicketChat'];
                }
                if(!empty($request['ms'])){
                    parse_str($request['ms'], $search['ms']);
                    if($_SESSION['memoire']['search']['formMsTicketTicketChat'.$IdSplit] != $request['ms']){
                        unset($_SESSION['memoire']['onglet']['TicketTicketChat']['pg']);
                    }
                    $_SESSION['memoire']['search']['formMsTicketTicketChat'.$IdSplit] = $request['ms'];
                }else if(!empty($_SESSION['memoire']['search']['formMsTicketTicketChat'.$IdSplit])){
                    parse_str($_SESSION['memoire']['search']['formMsTicketTicketChat'.$IdSplit], $search['ms']);
                }
                if(!empty($request['pg'])){ $_SESSION['memoire']['onglet']['TicketTicketChat']['pg'] = $request['pg'];}
                
                $f = new TicketForm();
                $f->TicketChat['request'] = $request;
                if(!empty($request['ip'])){
                    $f->TicketChat['parentId'] = $request['ip'];
                    $data['IdTicket'] = $request['ip'];
                }
                
                if(!empty($request['noHeader'])){$params['noHeader'] = $request['noHeader'];}
                $_SESSION['CurrentParent'] = 'Ticket';
                $childTableData = $f->getTicketChatList(@$request['ip'],$request['pg'], @$request['ui'], @$request['pui'], true, @$search, @$params);
                
                unset($_SESSION['CurrentParent']);
                @$output['html'] .= div(@$f->CcTicketChatTopList.$childTableData['html'].@$f->CcTicketChatBottomList, 'TicketChatTableCntnr','class="child-wrapper"');
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Ticket",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Ticket",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Ticket",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Ticket",'r')){
            init_list_base($request,$search,'Ticket','','','','');
            $e = new TicketForm();
            $e->Ticket['request'] = $request;
            if(!empty($request['ip'])){$e->Ticket['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderTicket']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsTicket'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Ticket($data, $request=array()){
        $e = new TicketForm();
        $e->Ticket['request'] = $request;
        return $e->save_update_Ticket($data);
    }
    function save_create_Ticket($data, $request=array()){
        $e = new TicketForm();
        $e->Ticket['request'] = $request;
        return $e->save_create_Ticket($data);
    }
