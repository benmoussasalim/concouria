...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = TemoignageQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Temoignage();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Temoignage';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_Temoignage($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        try{
                            $e->setLocale('fr_CA');
                            if(isset($data['TemoignageI18n_Text_frCA'])){$e->setText($data['TemoignageI18n_Text_frCA']);}
                            if(isset($data['TemoignageI18n_Resume_frCA'])){$e->setResume($data['TemoignageI18n_Resume_frCA']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new TemoignageI18n();
                            $mt->setLocale('fr_CA');
                            
                            if(isset($data['TemoignageI18n_Text_frCA'])){$mt->setText($data['TemoignageI18n_Text_frCA']);}
                            if(isset($data['TemoignageI18n_Resume_frCA'])){$mt->setResume($data['TemoignageI18n_Resume_frCA']);}
                            $e->addTemoignageI18n($mt);$e->save();
                        }
                    
                        try{
                            $e->setLocale('en_US');
                            if(isset($data['TemoignageI18n_Text_enUS'])){$e->setText($data['TemoignageI18n_Text_enUS']);}
                            if(isset($data['TemoignageI18n_Resume_enUS'])){$e->setResume($data['TemoignageI18n_Resume_enUS']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new TemoignageI18n();
                            $mt->setLocale('en_US');
                            
                            if(isset($data['TemoignageI18n_Text_enUS'])){$mt->setText($data['TemoignageI18n_Text_enUS']);}
                            if(isset($data['TemoignageI18n_Resume_enUS'])){$mt->setResume($data['TemoignageI18n_Resume_enUS']);}
                            $e->addTemoignageI18n($mt);$e->save();
                        }
                    
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Témoignage'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Témoignage'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_Temoignage($data, $request);
                    
                    
                        
                        
                        
                    $e->setLocale('fr_CA');$e->setText($data['TemoignageI18n_Text_frCA']);
                    $e->setLocale('fr_CA');$e->setResume($data['TemoignageI18n_Resume_frCA']);
                    $e->setLocale('en_US');$e->setText($data['TemoignageI18n_Text_enUS']);
                    $e->setLocale('en_US');$e->setResume($data['TemoignageI18n_Resume_enUS']);
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        $e->save();
                        
                        unset($_SESSION['memoire']['onglet']['Temoignage']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Temoignage']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Temoignage']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Temoignage']['i'] = set_var($_SESSION['memoire']['onglet']['Temoignage']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Témoignage'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Temoignage';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdTemoignage'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new TemoignageForm();
                                $e->Temoignage['request'] = $request;
                                $e->Temoignage['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new TemoignageForm();
                $f->Temoignage['request'] = $request;
                if(!empty($request['ip'])){$f->Temoignage['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Temoignage']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Temoignage']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdTemoignage';
                    ...
                    unset($_SESSION['memoire']['onglet']['Temoignage']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = TemoignageQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Temoignage';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Temoignage",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Temoignage",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Temoignage",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Temoignage",'r')){
            init_list_base($request,$search,'Temoignage','','','','');
            $e = new TemoignageForm();
            $e->Temoignage['request'] = $request;
            if(!empty($request['ip'])){$e->Temoignage['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderTemoignage']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsTemoignage'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Temoignage($data, $request=array()){
        $e = new TemoignageForm();
        $e->Temoignage['request'] = $request;
        return $e->save_update_Temoignage($data);
    }
    function save_create_Temoignage($data, $request=array()){
        $e = new TemoignageForm();
        $e->Temoignage['request'] = $request;
        return $e->save_create_Temoignage($data);
    }
