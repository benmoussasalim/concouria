...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = BlockI18nQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new BlockI18n();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'BlockI18n';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_BlockI18n($data, $request);
                    ...
                    $e = save_create_BlockI18n($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['BlockI18n']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['BlockI18n']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['BlockI18n']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['BlockI18n']['i'] = set_var($_SESSION['memoire']['onglet']['BlockI18n']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'BlockI18n';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdBlock'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new BlockI18nForm();
                                $e->BlockI18n['request'] = $request;
                                $e->BlockI18n['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new BlockI18nForm();
                $f->BlockI18n['request'] = $request;
                if(!empty($request['ip'])){$f->BlockI18n['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['BlockI18n']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['BlockI18n']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    ...
                    unset($_SESSION['memoire']['onglet']['BlockI18n']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = BlockI18nQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'BlockI18n';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."BlockI18n",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."BlockI18n",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."BlockI18n",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("BlockI18n",'r')){
            init_list_base($request,$search,'BlockI18n','','','','');
            $e = new BlockI18nForm();
            $e->BlockI18n['request'] = $request;
            if(!empty($request['ip'])){$e->BlockI18n['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderBlockI18n']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsBlockI18n'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_BlockI18n($data, $request=array()){
        $e = new BlockI18nForm();
        $e->BlockI18n['request'] = $request;
        return $e->save_update_BlockI18n($data);
    }
    function save_create_BlockI18n($data, $request=array()){
        $e = new BlockI18nForm();
        $e->BlockI18n['request'] = $request;
        return $e->save_create_BlockI18n($data);
    }
