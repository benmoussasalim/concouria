...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = PaysI18nQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new PaysI18n();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'PaysI18n';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_PaysI18n($data, $request);
                    ...
                    $e = save_create_PaysI18n($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['PaysI18n']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['PaysI18n']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['PaysI18n']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['PaysI18n']['i'] = set_var($_SESSION['memoire']['onglet']['PaysI18n']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'PaysI18n';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new PaysI18nForm();
                                $e->PaysI18n['request'] = $request;
                                $e->PaysI18n['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new PaysI18nForm();
                $f->PaysI18n['request'] = $request;
                if(!empty($request['ip'])){$f->PaysI18n['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['PaysI18n']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['PaysI18n']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    ...
                    unset($_SESSION['memoire']['onglet']['PaysI18n']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = PaysI18nQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'PaysI18n';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."PaysI18n",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."PaysI18n",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."PaysI18n",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("PaysI18n",'r')){
            init_list_base($request,$search,'PaysI18n','','','','');
            $e = new PaysI18nForm();
            $e->PaysI18n['request'] = $request;
            if(!empty($request['ip'])){$e->PaysI18n['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderPaysI18n']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsPaysI18n'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_PaysI18n($data, $request=array()){
        $e = new PaysI18nForm();
        $e->PaysI18n['request'] = $request;
        return $e->save_update_PaysI18n($data);
    }
    function save_create_PaysI18n($data, $request=array()){
        $e = new PaysI18nForm();
        $e->PaysI18n['request'] = $request;
        return $e->save_create_PaysI18n($data);
    }
