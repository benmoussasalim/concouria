...
###############################
/**
 * Form class on the 'MonthWinnerI18n' table.
 *
 */
class MonthWinnerI18nForm extends MonthWinnerI18n{
public $tableName="MonthWinnerI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('MonthWinnerI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."MonthWinnerI18n/edit/", "id='addMonthWinnerI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addMonthWinnerI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['MonthWinnerI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['MonthWinnerI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'MonthWinnerI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'MonthWinnerI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#MonthWinnerI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#MonthWinnerI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('MonthWinnerI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('MonthWinnerI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('MonthWinnerI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteMonthWinnerI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='MonthWinnerI18nRow".$data->getPrimaryKey()."'
                    data-table='MonthWinnerI18n' 
            ...
            $tr .= input('hidden', 'rowCountMonthWinnerI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'MonthWinnerI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='MonthWinnerI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
                ,'listForm',' class="ac-list contentHolderListP" ')
                .$this->CcToListBottom
                .@$bottomRow
            , 'MonthWinnerI18nListForm');
        
        $hasParent = json_decode($IdParent);
        if(empty($hasParent)){
            $editEvent = "
        $(\"#MonthWinnerI18nListForm [j='deleteMonthWinnerI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msMonthWinnerI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msMonthWinnerI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#MonthWinnerI18nListForm tr[ecf=1] td[j='editMonthWinnerI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."MonthWinnerI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."MonthWinnerI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#MonthWinnerI18nListForm [j='deleteMonthWinnerI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msMonthWinnerI18nBt\').length>0){ $(\'#msMonthWinnerI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'MonthWinnerI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntMonthWinnerI18nListForm #addMonthWinnerI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'MonthWinnerI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#MonthWinnerI18nListForm tr[ecf=1] td[j='editMonthWinnerI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'MonthWinnerI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('MonthWinnerI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#MonthWinnerI18nListForm [j='deleteMonthWinnerI18n']\").unbind('click');
        $('#MonthWinnerI18nListForm #addMonthWinnerI18n').unbind('click');
        $(\"#MonthWinnerI18nListForm tr[ecf=1] td[j='editMonthWinnerI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#MonthWinnerI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('MonthWinnerI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('MonthWinnerI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_MonthWinnerI18n($data){

        unset($data['IdMonthWinner']);
        $e = new MonthWinnerI18n();
    ...
    public function save_update_MonthWinnerI18n($data){

        
        $e = MonthWinnerI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of MonthWinnerI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['MonthWinnerI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['MonthWinnerI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addMonthWinnerI18n_child').bind('click.addMonthWinnerI18n', function (){
                    $.post('"._SITE_URL."mod/act/MonthWinnerI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addMonthWinnerI18n_child').bind('click.addMonthWinnerI18n', function (){
                document.location= '"._SITE_URL."MonthWinnerI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'MonthWinnerI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'MonthWinnerI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formMonthWinnerI18n #saveMonthWinnerI18n').removeAttr('disabled');
                    $('#formMonthWinnerI18n #saveMonthWinnerI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formMonthWinnerI18n #saveMonthWinnerI18n').css('cursor', 'default');
                    if($('#formMonthWinnerI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formMonthWinnerI18n #saveMonthWinnerI18n').bind('click.saveMonthWinnerI18n', function (data){
                    $('#formMonthWinnerI18n #saveMonthWinnerI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formMonthWinnerI18n #saveMonthWinnerI18n').css('cursor', 'progress');
                    $('#formMonthWinnerI18n #saveMonthWinnerI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formMonthWinnerI18n .tinymce').each(function(index) {
                        eval(' $(\"#formMonthWinnerI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formMonthWinnerI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formMonthWinnerI18n\").find(\"[s='d']\").serializeArray();
                        $('#formMonthWinnerI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formMonthWinnerI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/MonthWinnerI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formMonthWinnerI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formMonthWinnerI18n #formChangedMonthWinnerI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formMonthWinnerI18n #saveMonthWinnerI18n').unbind('click.saveMonthWinnerI18n');
                $('#formMonthWinnerI18n #saveMonthWinnerI18n').remove();";
        }
        
        if($dataObj == null){
            $this->MonthWinnerI18n['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new MonthWinnerI18n();
            $this->MonthWinnerI18n['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->MonthWinnerI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdMonthWinner()){
            $this->printLink =
                ...
                ,'MonthWinnerI18nControls', "class='custom-controls'");
        ...
        $this->formTitle = p(href("",_SITE_URL.'MonthWinnerI18n').$titre_form_str, 'class="breadcrumb"'); 
        $this->add_button= '';
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            
            $this->add_button = 
                
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addMonthWinnerI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'MonthWinnerI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formMonthWinnerI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['MonthWinnerI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['MonthWinnerI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['MonthWinnerI18n']['Start']['html']
                
                
                
                .@$this->fields['MonthWinnerI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntMonthWinnerI18n", "class='divStdform' CntTabs=1 ")
        , "id='formMonthWinnerI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdMonthWinner()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelMonthWinnerI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntMonthWinnerI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['MonthWinnerI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['MonthWinnerI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['MonthWinnerI18n']['ixmemautocapp']);
        }
    ...
    ".$jqueryDatePicker."
    bind_ui_active('MonthWinnerI18n');
    ".$this->AddButtonJs."
    ".$this->SaveButtonJs."
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntMonthWinnerI18n');},500); 
    ".$toggleForm."
    bind_form('MonthWinnerI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['MonthWinnerI18n'] as $field=>$ar){
                $this->fields['MonthWinnerI18n'][$field]['html'] = $this->fieldsRo['MonthWinnerI18n'][$field]['html'];
            }
    ...
    */
    ...
    $this->ajaxPageAct = 'MonthWinnerI18nAct.php';
