...
###############################
    ...
    $CurrentClass= 'Concours';
    
    
    switch($request['a']){
        
        case 'del_all_Concours':
            $_SESSION['CurrentRights']='Concours';
            parse_str($request['i'],$ids);
            if($ids){ ConcoursQuery::create()->filterByIdConcours($ids)->delete();}
        ...
        break;
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = ConcoursQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Concours();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        case 'bulkUpdateForm':
            if($_SESSION[_AUTH_VAR]->hasRights("Concours",'r')){
                $f = new ConcoursForm();
                $f->Concours['request'] = $request;
                $f->Concours['parentId'] = $request['ip'];
                parse_str($request['i'], $data);
                $data['i'] = json_encode($data);
                $output = $f->bulkUpdateForm($data,$request['ui']);
            }
        break;
        case 'BUsave':
            parse_str($request['d'], $data);
            if($data['idPk']){$data['ids'] = json_decode(urldecode($data['idPk']), true);}
            if(is_array($data['ids'])){
                if($_SESSION[_AUTH_VAR]->hasRights('Concours', 'b')){
                    $_SESSION['CurrentRights']='Concours';
                    foreach($data['ids'] as $id){
                        ...
                        $e = ConcoursQuery::create()->findPk($data['i']);

                        $e->setIdModification($_SESSION[_AUTH_VAR]->get('id'));
                        $e->setDateModification( date('Y-m-d H:i:s'));
                        
            if($data['ck_Online'] == 'up'){
                if($data['Online']==''){$data['Online'] = NULL;}
                $e->setOnline($data['Online']);
            }
                        ...
                        if ($e->validate() && !$extValidationErr) {
                            ...
                            $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Concours'));
                            
                            $updated[] = $e->getPrimaryKey();
                        }else{
                            $error = handleValidationError($e, $request['ui'],_('Concours'), $extValidationErr);
                            $previousData = $data;
                        }
                        
                    }
                    $return['ok'] = 'ok';
                    $return['updated'] = json_encode($updated);
                }
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Concours';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_Concours($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        try{
                            $e->setLocale('fr_CA');
                            if(isset($data['ConcoursI18n_Name_frCA'])){$e->setName($data['ConcoursI18n_Name_frCA']);}
                            if(isset($data['ConcoursI18n_Text_frCA'])){$e->setText($data['ConcoursI18n_Text_frCA']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new ConcoursI18n();
                            $mt->setLocale('fr_CA');
                            
                            if(isset($data['ConcoursI18n_Name_frCA'])){$mt->setName($data['ConcoursI18n_Name_frCA']);}
                            if(isset($data['ConcoursI18n_Text_frCA'])){$mt->setText($data['ConcoursI18n_Text_frCA']);}
                            $e->addConcoursI18n($mt);$e->save();
                        }
                    
                        try{
                            $e->setLocale('en_US');
                            if(isset($data['ConcoursI18n_Name_enUS'])){$e->setName($data['ConcoursI18n_Name_enUS']);}
                            if(isset($data['ConcoursI18n_Text_enUS'])){$e->setText($data['ConcoursI18n_Text_enUS']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new ConcoursI18n();
                            $mt->setLocale('en_US');
                            
                            if(isset($data['ConcoursI18n_Name_enUS'])){$mt->setName($data['ConcoursI18n_Name_enUS']);}
                            if(isset($data['ConcoursI18n_Text_enUS'])){$mt->setText($data['ConcoursI18n_Text_enUS']);}
                            $e->addConcoursI18n($mt);$e->save();
                        }
                    ...
                    $e = save_create_Concours($data, $request);
                    
                    
                        
                        
                        
                    $e->setLocale('fr_CA');$e->setName($data['ConcoursI18n_Name_frCA']);
                    $e->setLocale('fr_CA');$e->setText($data['ConcoursI18n_Text_frCA']);
                    $e->setLocale('en_US');$e->setName($data['ConcoursI18n_Name_enUS']);
                    $e->setLocale('en_US');$e->setText($data['ConcoursI18n_Text_enUS']);
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        $e->save();
                        
                        unset($_SESSION['memoire']['onglet']['Concours']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Concours']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Concours']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Concours']['i'] = set_var($_SESSION['memoire']['onglet']['Concours']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Concours'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Concours';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new ConcoursForm();
                                $e->Concours['request'] = $request;
                                $e->Concours['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new ConcoursForm();
                $f->Concours['request'] = $request;
                if(!empty($request['ip'])){$f->Concours['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Concours']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Concours']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    ...
                    unset($_SESSION['memoire']['onglet']['Concours']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = ConcoursQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Concours';
                if(!$obj->delete()){
        ...
        case 'getConcoursFileList':
            if($_SESSION[_AUTH_VAR]->hasRights("ConcoursConcoursFile",'r')){
                $listChild=true;

                $_SESSION['memoire']['onglet']['Concours']['child']['list'][$request['ip']]= 'ConcoursFile';
                if(!empty($request['ip'])){$IdCurrentConcours = $request['ip'];}
                $IdSplit=""; 
                if(isset($request['ms']) and $request['ms'] == 'clear'){
                    $request['ms'] = '';$request['order'] = '';
                    unset($_SESSION['memoire']['search']['formMsConcoursConcoursFile'.$IdSplit]);
                    unset($_SESSION['memoire']['search']['formOrderConcoursConcoursFile']);
                    unset($_SESSION['memoire']['onglet']['ConcoursConcoursFile']['pg']);
                    
                }
                if(!empty($request['order'])){
                    $search['order'] = json_decode($request['order'], true);
                    $_SESSION['memoire']['search']['formOrderConcoursConcoursFile'][$search['order']['col']] = $search['order']['sens'];
                    $search['order']= $_SESSION['memoire']['search']['formOrderConcoursConcoursFile'];
                }else if(!empty($_SESSION['memoire']['search']['formOrderConcoursConcoursFile'])){
                    $search['order']=$_SESSION['memoire']['search']['formOrderConcoursConcoursFile'];
                }
                if(!empty($request['ms'])){
                    parse_str($request['ms'], $search['ms']);
                    if($_SESSION['memoire']['search']['formMsConcoursConcoursFile'.$IdSplit] != $request['ms']){
                        unset($_SESSION['memoire']['onglet']['ConcoursConcoursFile']['pg']);
                    }
                    $_SESSION['memoire']['search']['formMsConcoursConcoursFile'.$IdSplit] = $request['ms'];
                }else if(!empty($_SESSION['memoire']['search']['formMsConcoursConcoursFile'.$IdSplit])){
                    parse_str($_SESSION['memoire']['search']['formMsConcoursConcoursFile'.$IdSplit], $search['ms']);
                }
                if(!empty($request['pg'])){ $_SESSION['memoire']['onglet']['ConcoursConcoursFile']['pg'] = $request['pg'];}
                
                $f = new ConcoursForm();
                $f->ConcoursFile['request'] = $request;
                if(!empty($request['ip'])){
                    $f->ConcoursFile['parentId'] = $request['ip'];
                    $data['IdConcours'] = $request['ip'];
                }
                
                if(!empty($request['noHeader'])){$params['noHeader'] = $request['noHeader'];}
                $_SESSION['CurrentParent'] = 'Concours';
                $childTableData = $f->getConcoursFileList(@$request['ip'],$request['pg'], @$request['ui'], @$request['pui'], true, @$search, @$params);
                
                unset($_SESSION['CurrentParent']);
                @$output['html'] .= div(@$f->CcConcoursFileTopList.$childTableData['html'].@$f->CcConcoursFileBottomList, 'ConcoursFileTableCntnr','class="child-wrapper"');
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Concours",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Concours",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Concours",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Concours",'r')){
            init_list_base($request,$search,'Concours','','','','');
            $e = new ConcoursForm();
            $e->Concours['request'] = $request;
            if(!empty($request['ip'])){$e->Concours['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderConcours']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsConcours'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Concours($data, $request=array()){
        $e = new ConcoursForm();
        $e->Concours['request'] = $request;
        return $e->save_update_Concours($data);
    }
    function save_create_Concours($data, $request=array()){
        $e = new ConcoursForm();
        $e->Concours['request'] = $request;
        return $e->save_create_Concours($data);
    }
