...
###############################
/**
 * Form class on the 'LabelI18n' table.
 *
 */
class LabelI18nForm extends LabelI18n{
public $tableName="LabelI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('LabelI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."LabelI18n/edit/", "id='addLabelI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addLabelI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['LabelI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['LabelI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'LabelI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'LabelI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#LabelI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#LabelI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('LabelI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('LabelI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('LabelI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteLabelI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='LabelI18nRow".$data->getPrimaryKey()."'
                    data-table='LabelI18n' 
            ...
            $tr .= input('hidden', 'rowCountLabelI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'LabelI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='LabelI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
        ...
        if(empty($hasParent)){
            $editEvent = "
        $(\"#LabelI18nListForm [j='deleteLabelI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msLabelI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msLabelI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#LabelI18nListForm tr[ecf=1] td[j='editLabelI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."LabelI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."LabelI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#LabelI18nListForm [j='deleteLabelI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msLabelI18nBt\').length>0){ $(\'#msLabelI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'LabelI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntLabelI18nListForm #addLabelI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'LabelI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#LabelI18nListForm tr[ecf=1] td[j='editLabelI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'LabelI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('LabelI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#LabelI18nListForm [j='deleteLabelI18n']\").unbind('click');
        $('#LabelI18nListForm #addLabelI18n').unbind('click');
        $(\"#LabelI18nListForm tr[ecf=1] td[j='editLabelI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#LabelI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('LabelI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('LabelI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_LabelI18n($data){
    ...
    public function save_update_LabelI18n($data){

        
        $e = LabelI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of LabelI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['LabelI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['LabelI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addLabelI18n_child').bind('click.addLabelI18n', function (){
                    $.post('"._SITE_URL."mod/act/LabelI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addLabelI18n_child').bind('click.addLabelI18n', function (){
                document.location= '"._SITE_URL."LabelI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'LabelI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'LabelI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formLabelI18n #saveLabelI18n').removeAttr('disabled');
                    $('#formLabelI18n #saveLabelI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formLabelI18n #saveLabelI18n').css('cursor', 'default');
                    if($('#formLabelI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formLabelI18n #saveLabelI18n').bind('click.saveLabelI18n', function (data){
                    $('#formLabelI18n #saveLabelI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formLabelI18n #saveLabelI18n').css('cursor', 'progress');
                    $('#formLabelI18n #saveLabelI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formLabelI18n .tinymce').each(function(index) {
                        eval(' $(\"#formLabelI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formLabelI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formLabelI18n\").find(\"[s='d']\").serializeArray();
                        $('#formLabelI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formLabelI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/LabelI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formLabelI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formLabelI18n #formChangedLabelI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formLabelI18n #saveLabelI18n').unbind('click.saveLabelI18n');
                $('#formLabelI18n #saveLabelI18n').remove();";
        }
        
        if($dataObj == null){
            $this->LabelI18n['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new LabelI18n();
            $this->LabelI18n['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->LabelI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdLabel()){
            $this->printLink =
                ...
                ,'LabelI18nControls', "class='custom-controls'");
        ...
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            ...
            $this->add_button = 
                ...
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addLabelI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'LabelI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formLabelI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['LabelI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['LabelI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['LabelI18n']['Start']['html']
                
                
                
                .@$this->fields['LabelI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntLabelI18n", "class='divStdform' CntTabs=1 ")
        , "id='formLabelI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdLabel()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelLabelI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntLabelI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['LabelI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['LabelI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['LabelI18n']['ixmemautocapp']);
        }
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntLabelI18n');},500); 
    ".$toggleForm."
    bind_form('LabelI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['LabelI18n'] as $field=>$ar){
                $this->fields['LabelI18n'][$field]['html'] = $this->fieldsRo['LabelI18n'][$field]['html'];
            }
    ...
    */
    ...
    $this->ajaxPageAct = 'LabelI18nAct.php';
