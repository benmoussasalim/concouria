...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = SaleQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Sale();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Sale';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_Sale($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Abonnement'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Abonnement'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_Sale($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['Sale']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Sale']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Sale']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Sale']['i'] = set_var($_SESSION['memoire']['onglet']['Sale']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);

                        }
                        
                        
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $data['IdAccount'] = json_encode($e->getIdAccount());
                        
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Abonnement'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Sale';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        ...
                        if(!$data['IdAccount'] && !$request['ip'] or ($request['je'] =='SaleTable' and !$request['ip']) ){
                            $output['onReadyJs'] .= "$('#formMsSale #msSaleBt').click();".$closeDiag."";
                        }else{
                            $output['onReadyJs'] .= "$('.child_pannel [p=\"Sale\"]').click();".$closeDiag."";
                        }
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new SaleForm();
                                $e->Sale['request'] = $request;
                                $e->Sale['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                if(!empty($request['ip'])){$f->Sale['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Sale']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Sale']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdSale';
                    ...
                    unset($_SESSION['memoire']['onglet']['Sale']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $obj = SaleQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Sale';
                if(!$obj->delete()){
        ...
        case 'getAbonnementList':
            if($_SESSION[_AUTH_VAR]->hasRights("SaleAbonnement",'r')){
                $listChild=true;

                $_SESSION['memoire']['onglet']['Sale']['child']['list'][$request['ip']]= 'Abonnement';
                if(!empty($request['ip'])){$IdCurrentSale = $request['ip'];}
                $IdSplit=""; 
                if(isset($request['ms']) and $request['ms'] == 'clear'){
                    $request['ms'] = '';$request['order'] = '';
                    unset($_SESSION['memoire']['search']['formMsSaleAbonnement'.$IdSplit]);
                    unset($_SESSION['memoire']['search']['formOrderSaleAbonnement']);
                    unset($_SESSION['memoire']['onglet']['SaleAbonnement']['pg']);
                    
                }
                if(!empty($request['order'])){
                    $search['order'] = json_decode($request['order'], true);
                    $_SESSION['memoire']['search']['formOrderSaleAbonnement'][$search['order']['col']] = $search['order']['sens'];
                    $search['order']= $_SESSION['memoire']['search']['formOrderSaleAbonnement'];
                }else if(!empty($_SESSION['memoire']['search']['formOrderSaleAbonnement'])){
                    $search['order']=$_SESSION['memoire']['search']['formOrderSaleAbonnement'];
                }
                if(!empty($request['ms'])){
                    parse_str($request['ms'], $search['ms']);
                    if($_SESSION['memoire']['search']['formMsSaleAbonnement'.$IdSplit] != $request['ms']){
                        unset($_SESSION['memoire']['onglet']['SaleAbonnement']['pg']);
                    }
                    $_SESSION['memoire']['search']['formMsSaleAbonnement'.$IdSplit] = $request['ms'];
                }else if(!empty($_SESSION['memoire']['search']['formMsSaleAbonnement'.$IdSplit])){
                    parse_str($_SESSION['memoire']['search']['formMsSaleAbonnement'.$IdSplit], $search['ms']);
                }
                if(!empty($request['pg'])){ $_SESSION['memoire']['onglet']['SaleAbonnement']['pg'] = $request['pg'];}
                
                $f = new SaleForm();
                $f->Abonnement['request'] = $request;
                if(!empty($request['ip'])){
                    $f->Abonnement['parentId'] = $request['ip'];
                    $data['IdSale'] = $request['ip'];
                }
                
                if(!empty($request['noHeader'])){$params['noHeader'] = $request['noHeader'];}
                $_SESSION['CurrentParent'] = 'Sale';
                $childTableData = $f->getAbonnementList(@$request['ip'],$request['pg'], @$request['ui'], @$request['pui'], true, @$search, @$params);
                
                unset($_SESSION['CurrentParent']);
                @$output['html'] .= div(@$f->CcAbonnementTopList.$childTableData['html'].@$f->CcAbonnementBottomList, 'AbonnementTableCntnr','class="child-wrapper"');
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Sale",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Sale",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Sale",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Sale",'r')){
            init_list_base($request,$search,'Sale','','','','');
            $e = new SaleForm();
            $e->Sale['request'] = $request;
            if(!empty($request['ip'])){$e->Sale['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderSale']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsSale'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Sale($data, $request=array()){
        $e = new SaleForm();
        $e->Sale['request'] = $request;
        return $e->save_update_Sale($data);
    }
    function save_create_Sale($data, $request=array()){
        $e = new SaleForm();
        $e->Sale['request'] = $request;
        return $e->save_create_Sale($data);
    }
