...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = MenuQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Menu();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Menu';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_Menu($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Hiérarchie'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Hiérarchie'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_Menu($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['Menu']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Menu']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Menu']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Menu']['i'] = set_var($_SESSION['memoire']['onglet']['Menu']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Hiérarchie'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Menu';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new MenuForm();
                                $e->Menu['request'] = $request;
                                $e->Menu['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new MenuForm();
                $f->Menu['request'] = $request;
                if(!empty($request['ip'])){$f->Menu['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Menu']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Menu']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdMenu';
                    ...
                    unset($_SESSION['memoire']['onglet']['Menu']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = MenuQuery::create()->findPk($request['i']);
                
            if($obj->countContents()){ $error = handleNotOkResponse(_("Cette entrée ne peut être supprimée car elle est présentement utilisée dans")." 'Contenu'. ", '', true,'Hiérarchie'); die( $error['onReadyJs'] ); }
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Menu';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Menu",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Menu",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Menu",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Menu",'r')){
            init_list_base($request,$search,'Menu','','','','');
            $e = new MenuForm();
            $e->Menu['request'] = $request;
            if(!empty($request['ip'])){$e->Menu['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderMenu']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsMenu'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Menu($data, $request=array()){
        $e = new MenuForm();
        $e->Menu['request'] = $request;
        return $e->save_update_Menu($data);
    }
    function save_create_Menu($data, $request=array()){
        $e = new MenuForm();
        $e->Menu['request'] = $request;
        return $e->save_create_Menu($data);
    }
