    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = BlockFileQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new BlockFile();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        case 'ForceDl':
            if($_SESSION[_AUTH_VAR]->hasRights('BlockFile','r')
            || $_SESSION[_AUTH_VAR]->hasRights($request['pc']."BlockFile",'r')
            ){
                $dataObj= BlockFileQuery::create()->findPk($request['i']);
...
array('name'=>$_FILES["file"]["name"][$f],'size'=>$_FILES["file"]["size"][$f],'type'=>$_FILES["file"]["type"][$f],'tmp_name'=>$_FILES["file"]["tmp_name"][$f],'error'=>$_FILES["file"]["error"][$f]);
                    ...
                    if( $path_info["extension"] ){
                        ...
                        if($tabIp){
                            foreach(array_unique($tabIp) as $ip){
                                $e = save_create_BlockFile($data, $request);
                                $e->setIdBlock($ip);
                                $path_rep_fichier = 'mod/file/';
                                $path_fichier = 'mod/file/BlockFile/';
                                ...
                                if(@$data['error'] == ''){
                                    if($_GET['blob']==1){
                                        ...
                                        if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'BlockFile';}
                                        $e->fromArray($data);
                                        if ($e->validate()){
                                            $e->save();
            ...
            case 'save':
                ...
                if(!empty($pkPresent)){
                    ...
                    ## Save
                     ...
                     $e = save_update_BlockFile($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['BlockFile']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['BlockFile']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['BlockFile']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['BlockFile']['i'] = set_var($_SESSION['memoire']['onglet']['BlockFile']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);

                        }
                        
                        
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $data['IdBlock'] = json_encode($e->getIdBlock());
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'BlockFile';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        ...
                        if(!$data['IdBlock'] && !$request['ip'] or ($request['je'] =='BlockFileTable' and !$request['ip']) ){
                            $output['onReadyJs'] .= "$('#formMsBlockFile #msBlockFileBt').click();".$closeDiag."";
                        }else{
                            $output['onReadyJs'] .= "$('.child_pannel [p=\"BlockFile\"]').click();".$closeDiag."";
                        }
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new BlockFileForm();
                                $e->BlockFile['request'] = $request;
                                $e->BlockFile['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                if(!empty($request['ip'])){$f->BlockFile['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['BlockFile']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['BlockFile']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdBlockFile';
                    ...
                    unset($_SESSION['memoire']['onglet']['BlockFile']['ixmemautoc']);
                }
                ...
                $IdCurrentBlockFile = $data['i'];
                
            $q = BlockFileQuery::create()->filterByIdBlockFile($IdCurrentBlockFile)->setFormatter(ModelCriteria::FORMAT_ARRAY)->findOne();

            @$output['onReadyJs'] .= "
                $('#formBlockFile .divtr.readonly .divtd[in=\"inSize\"] label.readonly').text('".addslashes($q['Size'])."');
                $('#formBlockFile #Size').val('".addslashes($q['Size'])."');
                $('#formBlockFile .divtr.readonly .divtd[in=\"inName\"] label.readonly').text('".addslashes($q['Name'])."');
                $('#formBlockFile #Name').val('".addslashes($q['Name'])."');
            ";
            ...
            case 'delete':
                ...
                $obj = BlockFileQuery::create()->findPk($request['i']);
        ...
        if(is_file(_INSTALL_PATH.$obj->getFichier())){
            unlink(_INSTALL_PATH.$obj->getFichier());
                ...
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'BlockFile';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."BlockFile",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."BlockFile",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."BlockFile",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("BlockFile",'r')){
            init_list_base($request,$search,'BlockFile','','','','');
            $e = new BlockFileForm();
            $e->BlockFile['request'] = $request;
            if(!empty($request['ip'])){$e->BlockFile['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderBlockFile']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsBlockFile'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_BlockFile($data, $request=array()){
        $e = new BlockFileForm();
        $e->BlockFile['request'] = $request;
        return $e->save_update_BlockFile($data);
    }
    function save_create_BlockFile($data, $request=array()){
        $e = new BlockFileForm();
        $e->BlockFile['request'] = $request;
        return $e->save_create_BlockFile($data);
    }
