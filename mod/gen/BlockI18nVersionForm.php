...
###############################
/**
 * Form class on the 'BlockI18nVersion' table.
 *
 */
class BlockI18nVersionForm extends BlockI18nVersion{
public $tableName="BlockI18nVersion";
...
public $siteTitle;
    ...
    public function getListSearch($IdParent='', $search=''){
        ...
        $q = BlockI18nVersionQuery::create();
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('BlockI18nVersion', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."BlockI18nVersion/edit/", "id='addBlockI18nVersion' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addBlockI18nVersionAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['BlockI18nVersion']['pg'])){
            $page = $_SESSION['memoire']['onglet']['BlockI18nVersion']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'BlockI18nVersionAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'BlockI18nVersionAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#BlockI18nVersionListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#BlockI18nVersionListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('BlockI18nVersion', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('BlockI18nVersion', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('BlockI18nVersion', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteBlockI18nVersion' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='BlockI18nVersionRow".$data->getPrimaryKey()."'
                    data-table='BlockI18nVersion' 
            ...
            $tr .= input('hidden', 'rowCountBlockI18nVersion', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'BlockI18nVersionControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='BlockI18nVersionTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
                ,'listForm',' class="ac-list contentHolderListP" ')
                .$this->CcToListBottom
                .@$bottomRow
            , 'BlockI18nVersionListForm');
        
        $hasParent = json_decode($IdParent);
        if(empty($hasParent)){
            $editEvent = "
        $(\"#BlockI18nVersionListForm [j='deleteBlockI18nVersion']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msBlockI18nVersionBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msBlockI18nVersionBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#BlockI18nVersionListForm tr[ecf=1] td[j='editBlockI18nVersion']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."BlockI18nVersion/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."BlockI18nVersion/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#BlockI18nVersionListForm [j='deleteBlockI18nVersion']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msBlockI18nVersionBt\').length>0){ $(\'#msBlockI18nVersionBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'BlockI18nVersionTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntBlockI18nVersionListForm #addBlockI18nVersion').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'BlockI18nVersionTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#BlockI18nVersionListForm tr[ecf=1] td[j='editBlockI18nVersion']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'BlockI18nVersionTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('BlockI18nVersion','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#BlockI18nVersionListForm [j='deleteBlockI18nVersion']\").unbind('click');
        $('#BlockI18nVersionListForm #addBlockI18nVersion').unbind('click');
        $(\"#BlockI18nVersionListForm tr[ecf=1] td[j='editBlockI18nVersion']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#BlockI18nVersionListForm [j='button']\").unbind();   
        pagination_sorted_bind('BlockI18nVersion','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('BlockI18nVersion','".addslashes($variableAutoc)."');
    ...
    public function save_create_BlockI18nVersion($data){

        unset($data['IdBlockI18nVersion']);
        $e = new BlockI18nVersion();
    ...
    public function save_update_BlockI18nVersion($data){

        
        $e = BlockI18nVersionQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of BlockI18nVersion
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['BlockI18nVersion']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['BlockI18nVersion']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addBlockI18nVersion_child').bind('click.addBlockI18nVersion', function (){
                    $.post('"._SITE_URL."mod/act/BlockI18nVersionAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addBlockI18nVersion_child').bind('click.addBlockI18nVersion', function (){
                document.location= '"._SITE_URL."BlockI18nVersion/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'BlockI18nVersion', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'BlockI18nVersion','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formBlockI18nVersion #saveBlockI18nVersion').removeAttr('disabled');
                    $('#formBlockI18nVersion #saveBlockI18nVersion').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formBlockI18nVersion #saveBlockI18nVersion').css('cursor', 'default');
                    if($('#formBlockI18nVersion #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formBlockI18nVersion #saveBlockI18nVersion').bind('click.saveBlockI18nVersion', function (data){
                    $('#formBlockI18nVersion #saveBlockI18nVersion').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formBlockI18nVersion #saveBlockI18nVersion').css('cursor', 'progress');
                    $('#formBlockI18nVersion #saveBlockI18nVersion').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formBlockI18nVersion .tinymce').each(function(index) {
                        eval(' $(\"#formBlockI18nVersion #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formBlockI18nVersion select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formBlockI18nVersion\").find(\"[s='d']\").serializeArray();
                        $('#formBlockI18nVersion select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formBlockI18nVersion\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/BlockI18nVersionAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formBlockI18nVersion #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formBlockI18nVersion #formChangedBlockI18nVersion').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formBlockI18nVersion #saveBlockI18nVersion').unbind('click.saveBlockI18nVersion');
                $('#formBlockI18nVersion #saveBlockI18nVersion').remove();";
        }
        
        if($dataObj == null){
            $this->BlockI18nVersion['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new BlockI18nVersion();
            $this->BlockI18nVersion['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->BlockI18nVersion['request']['ChildHide']) ){
            
        }
        
        
        if($dataObj->getIdBlockI18nVersion()){
            $this->printLink =
                div(
                    ""
                    .@$this->CcPrintLinkDiv
                ...
                ,'BlockI18nVersionControls', "class='custom-controls'");
        ...
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            ...
            $this->add_button = 
                ...
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addBlockI18nVersion_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'BlockI18nVersionControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formBlockI18nVersion');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['BlockI18nVersion']['tog']) and 
            $_SESSION['memoire']['onglet']['BlockI18nVersion']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['BlockI18nVersion']['Start']['html']
                
                
                
                .@$this->fields['BlockI18nVersion']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntBlockI18nVersion", "class='divStdform' CntTabs=1 ")
        , "id='formBlockI18nVersion' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdBlockI18nVersion()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelBlockI18nVersion', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntBlockI18nVersionChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['BlockI18nVersion']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['BlockI18nVersion']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['BlockI18nVersion']['ixmemautocapp']);
        }
    ...
    ".$jqueryDatePicker."
    bind_ui_active('BlockI18nVersion');
    ".$this->AddButtonJs."
    ".$this->SaveButtonJs."
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntBlockI18nVersion');},500); 
    ".$toggleForm."
    bind_form('BlockI18nVersion','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['BlockI18nVersion'] as $field=>$ar){
                $this->fields['BlockI18nVersion'][$field]['html'] = $this->fieldsRo['BlockI18nVersion'][$field]['html'];
            }
    ...
    */
    public function getSelectOptions($IdParent='', $selected='',$arrayOnly=false){
        ...
        if(!empty($IdParent)){
             ...
             $q->select(array('Name', 'IdBlockI18nVersion'));
    ...
    $this->ajaxPageAct = 'BlockI18nVersionAct.php';
