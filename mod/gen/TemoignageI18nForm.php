...
###############################
/**
 * Form class on the 'TemoignageI18n' table.
 *
 */
class TemoignageI18nForm extends TemoignageI18n{
public $tableName="TemoignageI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('TemoignageI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."TemoignageI18n/edit/", "id='addTemoignageI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addTemoignageI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['TemoignageI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['TemoignageI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'TemoignageI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'TemoignageI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#TemoignageI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#TemoignageI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('TemoignageI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('TemoignageI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('TemoignageI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteTemoignageI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='TemoignageI18nRow".$data->getPrimaryKey()."'
                    data-table='TemoignageI18n' 
            ...
            $tr .= input('hidden', 'rowCountTemoignageI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'TemoignageI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='TemoignageI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
                ,'listForm',' class="ac-list contentHolderListP" ')
                .$this->CcToListBottom
                .@$bottomRow
            , 'TemoignageI18nListForm');
        
        $hasParent = json_decode($IdParent);
        if(empty($hasParent)){
            $editEvent = "
        $(\"#TemoignageI18nListForm [j='deleteTemoignageI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msTemoignageI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msTemoignageI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#TemoignageI18nListForm tr[ecf=1] td[j='editTemoignageI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."TemoignageI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."TemoignageI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#TemoignageI18nListForm [j='deleteTemoignageI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msTemoignageI18nBt\').length>0){ $(\'#msTemoignageI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'TemoignageI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntTemoignageI18nListForm #addTemoignageI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'TemoignageI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#TemoignageI18nListForm tr[ecf=1] td[j='editTemoignageI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'TemoignageI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('TemoignageI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#TemoignageI18nListForm [j='deleteTemoignageI18n']\").unbind('click');
        $('#TemoignageI18nListForm #addTemoignageI18n').unbind('click');
        $(\"#TemoignageI18nListForm tr[ecf=1] td[j='editTemoignageI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#TemoignageI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('TemoignageI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('TemoignageI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_TemoignageI18n($data){

        unset($data['IdTemoignage']);
        $e = new TemoignageI18n();
    ...
    public function save_update_TemoignageI18n($data){

        
        $e = TemoignageI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of TemoignageI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['TemoignageI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['TemoignageI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addTemoignageI18n_child').bind('click.addTemoignageI18n', function (){
                    $.post('"._SITE_URL."mod/act/TemoignageI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addTemoignageI18n_child').bind('click.addTemoignageI18n', function (){
                document.location= '"._SITE_URL."TemoignageI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'TemoignageI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'TemoignageI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formTemoignageI18n #saveTemoignageI18n').removeAttr('disabled');
                    $('#formTemoignageI18n #saveTemoignageI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formTemoignageI18n #saveTemoignageI18n').css('cursor', 'default');
                    if($('#formTemoignageI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formTemoignageI18n #saveTemoignageI18n').bind('click.saveTemoignageI18n', function (data){
                    $('#formTemoignageI18n #saveTemoignageI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formTemoignageI18n #saveTemoignageI18n').css('cursor', 'progress');
                    $('#formTemoignageI18n #saveTemoignageI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formTemoignageI18n .tinymce').each(function(index) {
                        eval(' $(\"#formTemoignageI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formTemoignageI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formTemoignageI18n\").find(\"[s='d']\").serializeArray();
                        $('#formTemoignageI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formTemoignageI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/TemoignageI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formTemoignageI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formTemoignageI18n #formChangedTemoignageI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formTemoignageI18n #saveTemoignageI18n').unbind('click.saveTemoignageI18n');
                $('#formTemoignageI18n #saveTemoignageI18n').remove();";
        }
        
        if($dataObj == null){
            $this->TemoignageI18n['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new TemoignageI18n();
            $this->TemoignageI18n['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->TemoignageI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdTemoignage()){
            $this->printLink =
                ...
                ,'TemoignageI18nControls', "class='custom-controls'");
        ...
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            ...
            $this->add_button = 
                ...
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addTemoignageI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'TemoignageI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formTemoignageI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['TemoignageI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['TemoignageI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['TemoignageI18n']['Start']['html']
                
                
                
                .@$this->fields['TemoignageI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntTemoignageI18n", "class='divStdform' CntTabs=1 ")
        , "id='formTemoignageI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdTemoignage()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelTemoignageI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntTemoignageI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['TemoignageI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['TemoignageI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['TemoignageI18n']['ixmemautocapp']);
        }
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntTemoignageI18n');},500); 
    ".$toggleForm."
    bind_form('TemoignageI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['TemoignageI18n'] as $field=>$ar){
                $this->fields['TemoignageI18n'][$field]['html'] = $this->fieldsRo['TemoignageI18n'][$field]['html'];
            }
    ...
    */
    ...
    $this->ajaxPageAct = 'TemoignageI18nAct.php';
