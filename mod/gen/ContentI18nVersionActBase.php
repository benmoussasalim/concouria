...
###############################
    ...
    $CurrentClass= 'ContentI18nVersion';
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = ContentI18nVersionQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new ContentI18nVersion();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'ContentI18nVersion';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_ContentI18nVersion($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_(''));
                    ...
                    $e = save_create_ContentI18nVersion($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['ContentI18nVersion']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['ContentI18nVersion']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['ContentI18nVersion']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['ContentI18nVersion']['i'] = set_var($_SESSION['memoire']['onglet']['ContentI18nVersion']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_(''));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'ContentI18nVersion';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdContentI18nVersion'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new ContentI18nVersionForm();
                                $e->ContentI18nVersion['request'] = $request;
                                $e->ContentI18nVersion['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new ContentI18nVersionForm();
                $f->ContentI18nVersion['request'] = $request;
                if(!empty($request['ip'])){$f->ContentI18nVersion['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['ContentI18nVersion']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['ContentI18nVersion']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdContentI18nVersion';
                    ...
                    unset($_SESSION['memoire']['onglet']['ContentI18nVersion']['ixmemautoc']);
                }
                ...
                $IdCurrentContentI18nVersion = $data['i'];
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = ContentI18nVersionQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'ContentI18nVersion';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."ContentI18nVersion",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."ContentI18nVersion",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."ContentI18nVersion",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("ContentI18nVersion",'r')){
            init_list_base($request,$search,'ContentI18nVersion','','','','');
            $e = new ContentI18nVersionForm();
            $e->ContentI18nVersion['request'] = $request;
            if(!empty($request['ip'])){$e->ContentI18nVersion['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderContentI18nVersion']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsContentI18nVersion'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_ContentI18nVersion($data, $request=array()){
        $e = new ContentI18nVersionForm();
        $e->ContentI18nVersion['request'] = $request;
        return $e->save_update_ContentI18nVersion($data);
    }
    function save_create_ContentI18nVersion($data, $request=array()){
        $e = new ContentI18nVersionForm();
        $e->ContentI18nVersion['request'] = $request;
        return $e->save_create_ContentI18nVersion($data);
    }
