...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = RegionQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Region();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Region';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_Region($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        try{
                            $e->setLocale('fr_CA');
                            if(isset($data['RegionI18n_Name_frCA'])){$e->setName($data['RegionI18n_Name_frCA']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new RegionI18n();
                            $mt->setLocale('fr_CA');
                            
                            if(isset($data['RegionI18n_Name_frCA'])){$mt->setName($data['RegionI18n_Name_frCA']);}
                            $e->addRegionI18n($mt);$e->save();
                        }
                    
                        try{
                            $e->setLocale('en_US');
                            if(isset($data['RegionI18n_Name_enUS'])){$e->setName($data['RegionI18n_Name_enUS']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new RegionI18n();
                            $mt->setLocale('en_US');
                            
                            if(isset($data['RegionI18n_Name_enUS'])){$mt->setName($data['RegionI18n_Name_enUS']);}
                            $e->addRegionI18n($mt);$e->save();
                        }
                    
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Région'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Région'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_Region($data, $request);
                    
                    
                        
                        
                        
                    $e->setLocale('fr_CA');$e->setName($data['RegionI18n_Name_frCA']);
                    $e->setLocale('en_US');$e->setName($data['RegionI18n_Name_enUS']);
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        $e->save();
                        
                        unset($_SESSION['memoire']['onglet']['Region']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Region']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Region']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Region']['i'] = set_var($_SESSION['memoire']['onglet']['Region']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Région'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Region';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdRegion'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new RegionForm();
                                $e->Region['request'] = $request;
                                $e->Region['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new RegionForm();
                $f->Region['request'] = $request;
                if(!empty($request['ip'])){$f->Region['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Region']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Region']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdRegion';
                    ...
                    unset($_SESSION['memoire']['onglet']['Region']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = RegionQuery::create()->findPk($request['i']);
                
            if($obj->countAccounts()){ $error = handleNotOkResponse(_("Cette entrée ne peut être supprimée car elle est présentement utilisée dans")." 'Compte'. ", '', true,'Région'); die( $error['onReadyJs'] ); }
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Region';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Region",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Region",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Region",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Region",'r')){
            init_list_base($request,$search,'Region','','','','');
            $e = new RegionForm();
            $e->Region['request'] = $request;
            if(!empty($request['ip'])){$e->Region['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderRegion']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsRegion'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Region($data, $request=array()){
        $e = new RegionForm();
        $e->Region['request'] = $request;
        return $e->save_update_Region($data);
    }
    function save_create_Region($data, $request=array()){
        $e = new RegionForm();
        $e->Region['request'] = $request;
        return $e->save_create_Region($data);
    }
