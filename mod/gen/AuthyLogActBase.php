...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = AuthyLogQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new AuthyLog();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'AuthyLog';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_AuthyLog($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Usagers Log'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Usagers Log'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_AuthyLog($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['AuthyLog']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['AuthyLog']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['AuthyLog']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['AuthyLog']['i'] = set_var($_SESSION['memoire']['onglet']['AuthyLog']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Usagers Log'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'AuthyLog';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdAuthyLog'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new AuthyLogForm();
                                $e->AuthyLog['request'] = $request;
                                $e->AuthyLog['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new AuthyLogForm();
                $f->AuthyLog['request'] = $request;
                if(!empty($request['ip'])){$f->AuthyLog['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['AuthyLog']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['AuthyLog']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdAuthyLog';
                    ...
                    unset($_SESSION['memoire']['onglet']['AuthyLog']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = AuthyLogQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'AuthyLog';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."AuthyLog",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."AuthyLog",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."AuthyLog",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("AuthyLog",'r')){
            init_list_base($request,$search,'AuthyLog','','','','');
            $e = new AuthyLogForm();
            $e->AuthyLog['request'] = $request;
            if(!empty($request['ip'])){$e->AuthyLog['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderAuthyLog']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsAuthyLog'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_AuthyLog($data, $request=array()){
        $e = new AuthyLogForm();
        $e->AuthyLog['request'] = $request;
        return $e->save_update_AuthyLog($data);
    }
    function save_create_AuthyLog($data, $request=array()){
        $e = new AuthyLogForm();
        $e->AuthyLog['request'] = $request;
        return $e->save_create_AuthyLog($data);
    }
