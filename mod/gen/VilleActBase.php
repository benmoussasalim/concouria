...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = VilleQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new Ville();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'Ville';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_Ville($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        try{
                            $e->setLocale('fr_CA');
                            if(isset($data['VilleI18n_Name_frCA'])){$e->setName($data['VilleI18n_Name_frCA']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new VilleI18n();
                            $mt->setLocale('fr_CA');
                            
                            if(isset($data['VilleI18n_Name_frCA'])){$mt->setName($data['VilleI18n_Name_frCA']);}
                            $e->addVilleI18n($mt);$e->save();
                        }
                    
                        try{
                            $e->setLocale('en_US');
                            if(isset($data['VilleI18n_Name_enUS'])){$e->setName($data['VilleI18n_Name_enUS']);}
                            $e->save();
                        }catch (Exception $f){
                            $mt = new VilleI18n();
                            $mt->setLocale('en_US');
                            
                            if(isset($data['VilleI18n_Name_enUS'])){$mt->setName($data['VilleI18n_Name_enUS']);}
                            $e->addVilleI18n($mt);$e->save();
                        }
                    
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Ville'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Ville'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_Ville($data, $request);
                    
                    
                        
                        
                        
                    $e->setLocale('fr_CA');$e->setName($data['VilleI18n_Name_frCA']);
                    $e->setLocale('en_US');$e->setName($data['VilleI18n_Name_enUS']);
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        $e->save();
                        
                        unset($_SESSION['memoire']['onglet']['Ville']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['Ville']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['Ville']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['Ville']['i'] = set_var($_SESSION['memoire']['onglet']['Ville']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);
                        ...
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Ville'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'Ville';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        $request['a'] = 'list';
                        $request['highlight'] .= '['.$data['IdVille'].']';
                    break;
                        case 'id_popup':
                        case 'sel':
                            $selectHtml = '';
                            ...
                            if(!$selectHtml){
                                $e = new VilleForm();
                                $e->Ville['request'] = $request;
                                $e->Ville['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new VilleForm();
                $f->Ville['request'] = $request;
                if(!empty($request['ip'])){$f->Ville['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['Ville']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['Ville']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdVille';
                    ...
                    unset($_SESSION['memoire']['onglet']['Ville']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $request['ot'] = 's';
                $obj = VilleQuery::create()->findPk($request['i']);
                
            if($obj->countAccounts()){ $error = handleNotOkResponse(_("Cette entrée ne peut être supprimée car elle est présentement utilisée dans")." 'Compte'. ", '', true,'Ville'); die( $error['onReadyJs'] ); }
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'Ville';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Ville",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Ville",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."Ville",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("Ville",'r')){
            init_list_base($request,$search,'Ville','','','','');
            $e = new VilleForm();
            $e->Ville['request'] = $request;
            if(!empty($request['ip'])){$e->Ville['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderVille']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsVille'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_Ville($data, $request=array()){
        $e = new VilleForm();
        $e->Ville['request'] = $request;
        return $e->save_update_Ville($data);
    }
    function save_create_Ville($data, $request=array()){
        $e = new VilleForm();
        $e->Ville['request'] = $request;
        return $e->save_create_Ville($data);
    }
