...
###############################
/**
 * Form class on the 'ProvinceI18n' table.
 *
 */
class ProvinceI18nForm extends ProvinceI18n{
public $tableName="ProvinceI18n";
...
public $siteTitle;
    ...
    public function getListHeader($act){
        switch($act){
            ...
            if(
                $_SESSION[_AUTH_VAR]->hasRights('ProvinceI18n', 'a') 
                ...
                if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                    $add_button = htmlLink(_("Ajouter"),_SITE_URL."ProvinceI18n/edit/", "id='addProvinceI18n' title='"._('Ajouter')."' class='button-link-blue add-button'");
                }else{
                    $add_button = div(
                htmlLink(span(_("Ajouter")),"javascript:", "id='addProvinceI18nAutoc' class='addButton'").$this->cCMainTableHeader, '', "class='listHeaderItem'");
                }
            }else{
                $add_button = '';
            }
    ...
    public function getList( $uiTabsId= "tabsContain", $page='1', $IdParent='', $pmpoDataIn='', $search=''){
        ...
        if(!empty($_SESSION['memoire']['onglet']['ProvinceI18n']['pg'])){
            $page = $_SESSION['memoire']['onglet']['ProvinceI18n']['pg'];
        }
        $this->uiTabsId = $uiTabsId;
        $this->ajaxPageActParent = (empty($this->cCajaxPageActParent))?'ProvinceI18nAct.php':$this->cCajaxPageActParent;
        $this->ajaxPageAct = (empty($this->cCajaxPageAct))?'ProvinceI18nAct.php':$this->cCajaxPageAct;
        ...
        if($this->searchOrder){
            foreach($this->searchOrder as $col=>$sens){
                if($sens){
                    $pOrder .="
                        $(\"#ProvinceI18nListForm [th='sorted'][c='".$col."']\").attr('sens','".strtolower($sens)."');
                         $(\"#ProvinceI18nListForm [th='sorted'][c='".$col."']\").attr('order','on');
                    ";
                }
            }
        }
        ...
        if(empty($pmpoData) || !method_exists($pmpoData,'isEmpty')){
            ...
            if(!isset($this->search['Autoc']['SelList']) || !$this->search['Autoc']['SelList']){
                ...
                $CheckAction = "";
                if(!$_SESSION[_AUTH_VAR]->hasRights('ProvinceI18n', 'd') && !$_SESSION[_AUTH_VAR]->hasRights('ProvinceI18n', 'b')){
                    $CheckAction ='';
                }

                if($_SESSION[_AUTH_VAR]->hasRights('ProvinceI18n', 'd')){
                    $this->ListActionRow = $this->ListActionRow.
        htmlLink(span(_("Supprimer"), 'class="ac-delete-label"'),"Javascript:","class='ac-delete-link' j='deleteProvinceI18n' i='".json_encode($data->getPrimaryKey())."'  ").$CheckAction;
                }else{
                ...
                */
                ...
                ," 
                    ...
                    class='".@$highlight." '
                    id='ProvinceI18nRow".$data->getPrimaryKey()."'
                    data-table='ProvinceI18n' 
            ...
            $tr .= input('hidden', 'rowCountProvinceI18n', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent.$this->CcCustomControl,'ProvinceI18nControlsList', "class='custom-controls ".@$hasControls."'")
            ...
            .div(
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        table($trHead.$tr, "id='ProvinceI18nTable' class='tablesorter' style='width:100%;'")
                     ,'',' class="content" ')
                ,'listForm',' class="ac-list contentHolderListP" ')
                .$this->CcToListBottom
                .@$bottomRow
            , 'ProvinceI18nListForm');
        
        $hasParent = json_decode($IdParent);
        if(empty($hasParent)){
            $editEvent = "
        $(\"#ProvinceI18nListForm [j='deleteProvinceI18n']\").bind('click', function (){
            $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msProvinceI18nBt\').length>0){ sw_message(\'".addslashes(_('Suppression complété.'))."\',false,\'search-progress\'); $(\'#msProvinceI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageActParent."\', {a:\'list\', ui:\'".$uiTabsId."\', p:$(this).attr(\'i\')}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
        });
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#ProvinceI18nListForm tr[ecf=1] td[j='editProvinceI18n']\").bind('click', function (e){
            if (e.which == 2) {  window.open('"._SITE_URL."ProvinceI18n/edit/'+$(this).attr('i'), '_blank'); }
            else { document.location = '"._SITE_URL."ProvinceI18n/edit/'+$(this).attr('i'); }
        });
        ";
            }else{
    ...
    ";
            ...
            $editEvent = "
        $(\"#ProvinceI18nListForm [j='deleteProvinceI18n']\").bind('click', function (){
                $('#ui-dialog-title-confirmDialog').html('".addslashes(_(''))."');confirm('".addslashes(message_label('supp_cette_entrer'))."',' $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'delete\', ui:\'".$uiTabsId."\', i:'+$(this).attr('i')+'}, function(data){ if(data){ $(\'#".$uiTabsId."\').append(data); }else if($(\'#msProvinceI18nBt\').length>0){ $(\'#msProvinceI18nBt\').click(); }else{ $.post(\'"._SITE_URL."mod/act/".$this->ajaxPageAct."\', {a:\'list\', ui:\'ProvinceI18nTable\', p:$(this).attr(\'i\'), ui:\'".$uiTabsId."\',ip:\'".json_encode($IdParent)."\'}, function(data){ $(\'#".$uiTabsId."\').html(data); }); } }); ');
            });

        $('#cntProvinceI18nListForm #addProvinceI18n').click(function(){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."',{a:'edit', ui:'editDialog', je:'ProvinceI18nTable', jet:'tr', ip:'".json_encode($IdParent)."'},function(data){
        ...
        ";
            if(empty($this->search['Autoc']['SelList'])){
                $editEvent .= "
        $(\"#ProvinceI18nListForm tr[ecf=1] td[j='editProvinceI18n']\").bind('click', function (){
            $.post('"._SITE_URL."mod/act/".$this->ajaxPageAct."', {a:'edit', i:$(this).attr('i'), ui:'editDialog', je:'ProvinceI18nTable', jet:'tr'},  function(data){
    ...
    ";
        ...
        $editEvent .= "
        pagination_bind('ProvinceI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','');";
        ...
        $(\"#ProvinceI18nListForm [j='deleteProvinceI18n']\").unbind('click');
        $('#ProvinceI18nListForm #addProvinceI18n').unbind('click');
        $(\"#ProvinceI18nListForm tr[ecf=1] td[j='editProvinceI18n']\").unbind('click');
    ".@$this->CcToListJsFirst.$editEvent."
        $(\"#ProvinceI18nListForm [j='button']\").unbind();   
        pagination_sorted_bind('ProvinceI18n','".$uiTabsId."','".addslashes($variableAutoc)."','".$this->ajaxPageActParent."','".$this->CcToSearchMsPost."');
        bind_js_retract();
        robot_mgr_add_autoc('ProvinceI18n','".addslashes($variableAutoc)."');
    ...
    public function save_create_ProvinceI18n($data){
    ...
    public function save_update_ProvinceI18n($data){

        
        $e = ProvinceI18nQuery::create()->findPk(json_decode($data['i']));
    ...
    /**
     * Produce a formated form of ProvinceI18n
    ...
    */
    public function getEditForm($id, $uiTabsId= "tabsContain", $data=array(), $error='', $jsElement='', $jsElementType='', $params=''){
        ...
        if($id and @$_SESSION['memoire']['onglet']['ProvinceI18n']['ogf']){
            $tabs_act = "$('[href=\"".$_SESSION['memoire']['onglet']['ProvinceI18n']['ogf']."\"]').click();";
        }
        $ip_save="";
        ################/* l'action doit savoir si st'en child ou pas */
        if($uiTabsId != "tabsContain"){
            $this->AddButtonJs = "
            $('#addProvinceI18n_child').bind('click.addProvinceI18n', function (){
                    $.post('"._SITE_URL."mod/act/ProvinceI18nAct.php', {a:'edit', ui:'".$editDialog."', je:'".$je."', jet:'".$jet."', ip:'".$data['ip']."',pc:'".$data['pc']."'},
                    function(data){  $('#".$uiTabsId."').html(data); });
            }); ";
            $ip_save = ",ip:'".json_encode($data['ip'])."'";
        }else{
              $this->AddButtonJs = "$('#addProvinceI18n_child').bind('click.addProvinceI18n', function (){
                document.location= '"._SITE_URL."ProvinceI18n/edit/';
            });";
        }
        ################
        ...
        if(($_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'ProvinceI18n', 'a') and !$id ) 
            || ( $_SESSION[_AUTH_VAR]->hasRights(@$data['req']['pc'].'ProvinceI18n','w',$dataObj)) 
            ...
            if(!empty($this->ccAfterSaveJs)){$ActionSaveJs =$this->ccAfterSaveJs;
                ...
                $ActionSaveJs ="
                    $('#formProvinceI18n #saveProvinceI18n').removeAttr('disabled');
                    $('#formProvinceI18n #saveProvinceI18n').removeClass('can-save');
                    $('body').css('cursor', 'default');
                    $('#formProvinceI18n #saveProvinceI18n').css('cursor', 'default');
                    if($('#formProvinceI18n #idPk').val() ==''){
                        $(\"#".$uiTabsId."\").html(data);
                    }else{ $(\"#".$uiTabsId."\").append(data); }
                ";
            }
            $this->SaveButtonJs = "
                $('#formProvinceI18n #saveProvinceI18n').bind('click.saveProvinceI18n', function (data){
                    $('#formProvinceI18n #saveProvinceI18n').attr('disabled', 'disabled');
                    
                    $('body').css('cursor', 'progress');
                    $('#formProvinceI18n #saveProvinceI18n').css('cursor', 'progress');
                    $('#formProvinceI18n #saveProvinceI18n').switchClass('ac-light-red','ac-light-blue',1000,'easeInOutQuad');
                    $('#formProvinceI18n .tinymce').each(function(index) {
                        eval(' $(\"#formProvinceI18n #'+$(this).attr('Id')+'\").val(CKEDITOR.instances.'+$(this).attr('Id')+'.getData());');
                    });
                    var strSerialize;
                    if($('#formProvinceI18n select[multiple]').length>0){
                        var strSerialize =$(this).parents(\"#formProvinceI18n\").find(\"[s='d']\").serializeArray();
                        $('#formProvinceI18n select[multiple]').each(function(){
                        ...
                        strSerialize =$(this).parents(\"#formProvinceI18n\").find(\"[s='d']\").serialize();
                    }
                    $.post('"._SITE_URL."mod/act/ProvinceI18nAct.php',{
                            'a':'save',d:strSerialize".$otherParam."
                            , ui:'".$uiTabsId."',pc:'".@$data['req']['pc']."',ip:'".@$data['ip']."', je:'".$jsElement."'
                            , jet:'".$jsElementType."'".$ip_save.", dialog:'".@$dialog."', tp:$('#formProvinceI18n #tp').val()
                        },
                        function(data){
                            ".$ActionSaveJs."
                        }
                    );$('#formProvinceI18n #formChangedProvinceI18n').val('');
                });
            ";
        }else{
            $this->setReadOnly='all';
            $this->SaveButtonJs = "
                $('#formProvinceI18n #saveProvinceI18n').unbind('click.saveProvinceI18n');
                $('#formProvinceI18n #saveProvinceI18n').remove();";
        }
        
        if($dataObj == null){
            $this->ProvinceI18n['isNew'] = 'yes';
        }
    
        
        
        if($dataObj == null){
            $dataObj = new ProvinceI18n();
            $this->ProvinceI18n['isNew'] = 'yes';
            if(is_array($data ))
        ...
        if(!isset($params['ChildHide'])  and !isset($this->ProvinceI18n['request']['ChildHide']) ){
        ...
        if($dataObj->getIdProvince()){
            $this->printLink =
                ...
                ,'ProvinceI18nControls', "class='custom-controls'");
        ...
        $this->formTitle = p(href("",_SITE_URL.'ProvinceI18n').$titre_form_str, 'class="breadcrumb"'); 
        $this->add_button= '';
        if($_SESSION[_AUTH_VAR]->hasRights($data['pc'].$this->tableName, 'a') && empty($this->setReadOnly)){
            
            $this->add_button = 
                
                htmlLink(_("Ajouter"), 'Javascript:;' , "id='addProvinceI18n_child' title='"._('Ajouter')."' class='button-link-blue add-button'");
        }
        ...
        //@@PRINTLINK
        if(!$this->printLink){ $this->printLink = div($this->CcCustomControl,'ProvinceI18nControls', "class='custom-controls'");}
        $jqueryDatePicker = "beforeDatePicker('#formProvinceI18n');";
        
        $toggleForm="";
        if(!empty($_SESSION['memoire']['onglet']['ProvinceI18n']['tog']) and 
            $_SESSION['memoire']['onglet']['ProvinceI18n']['tog']=='true'){$toggleForm="$('#toggle-form').click();";}
        $header_top = 
                div(
        ...
        .form(
            ...
            .div(
                ...
                .@$this->fields['ProvinceI18n']['Start']['html']
                
                
                
                .@$this->fields['ProvinceI18n']['End']['html']
                
                .@$this->CcToInnerFormBottom
            ,"divCntProvinceI18n", "class='divStdform' CntTabs=1 ")
        , "id='formProvinceI18n' data-pc='".addslashes($data['pc'])."' class='mainForm formContent ".@$this->CcToClassForm."' ")
        .$this->CcToFormBottom;
        ...
        if($dataObj->getIdProvince()){
            if(!empty($ChildOnglet)){
                $return['html'] .= div(ul($ChildOnglet, " class=' ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all' "), 'pannelProvinceI18n', " class='child_pannel ui-tabs ui-widget ui-widget-content ui-corner-all'");
                $return['html'] .= div('','cntProvinceI18nChild',' class="childCntClass" ');
            }
        }
        if(!empty($this->CcEvalToForm)){eval($this->CcEvalToForm);}
        if(!empty($_SESSION['memoire']['onglet']['ProvinceI18n']['ixmemautocapp']) and $_GET['Autocapp'] == 1){
            $Autocapp = $_SESSION['memoire']['onglet']['ProvinceI18n']['ixmemautocapp'];
            
            unset($_SESSION['memoire']['onglet']['ProvinceI18n']['ixmemautocapp']);
        }
    ...
    ".$this->CcToFormJs
    .@$script_autoc_one."
     setTimeout(function(){ bind_othertabs_std('#divCntProvinceI18n');},500); 
    ".$toggleForm."
    bind_form('ProvinceI18n','".addslashes($this->CcToFormEndJs)."');
    ";
        return $return;
    }
    ...
    function lockFormField($fields, $dataObj){
        ...
        if($fields == 'all'){
            foreach($this->fields['ProvinceI18n'] as $field=>$ar){
                $this->fields['ProvinceI18n'][$field]['html'] = $this->fieldsRo['ProvinceI18n'][$field]['html'];
            }
    ...
    */
    public function getSelectOptions($IdParent='', $selected='',$arrayOnly=false){
        ...
        if(!empty($IdParent)){
             ...
             $q->select(array('Name', 'IdProvince'));
    ...
    $this->ajaxPageAct = 'ProvinceI18nAct.php';
