...
###############################
    ...
    switch($request['a']){
            ...
            case 'selectBox':
                if($request['who'] AND $request['ctb_name'] and $request['ch_name'] and $request['chp_name']){
                    ...
                    if($whos){
                        $fw=0;
                        $dataObj = TicketChatQuery::create()->findPk($request['i']);
                        if(!$dataObj){ $dataObj = new TicketChat();}
                        foreach($whos as $who){
                            $func ="selectBox".$who;
        ...
        #	Main Object Setter
            case 'save':
                ...
                if($request['jet'] != 'id_popup'){ $_SESSION['CurrentRights'][]=$request['pc'].'TicketChat';}
                if(!empty($pkPresent)){
                    
                    ## Save
                    
                    
                     $e = save_update_TicketChat($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        $error = handleOkResponse(" "._('Enregistré à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Conversation'));
                        
                        
                    }else{
                        $error = handleValidationError($e, $request['ui'],_('Conversation'), $extValidationErr);
                        $previousData = $data;
                    }
                    ...
                    $e = save_create_TicketChat($data, $request);
                    ...
                    if ($e->validate() && !$extValidationErr && !@$error) {
                        ...
                        unset($_SESSION['memoire']['onglet']['TicketChat']['mem']);
                        $returnMem =rmv_var($_SESSION['memoire']['onglet']['TicketChat']['i'],0,',',false);
                        if($returnMem ){$_SESSION['memoire']['onglet']['TicketChat']['i']=$returnMem;}
                        if(!$data['ip']){
                               $_SESSION['memoire']['onglet']['TicketChat']['i'] = set_var($_SESSION['memoire']['onglet']['TicketChat']['i'],$e->getPrimaryKey(),',',false,true,_onglet_formulaire);

                        }
                        
                        
                        $data['i'] = json_encode($e->getPrimaryKey());
                        $data['IdTicket'] = json_encode($e->getIdTicket());
                        
                        if(@$_REQUEST['sa'] != 'qf'){
                            $error = handleOkResponse(" "._('Ajouté à')." <span>".date("H:i:s")."</span>",$request['ui'],'',_('Conversation'));
                        }
                    ...
                    if(!$request['pathEntite']){ $request['pathEntite'] = 'TicketChat';}
                }
                ...
                if(empty($error['error']) && !empty($request['je'])){
                    ...
                    case 'tr':
                        ...
                        if(!$data['IdTicket'] && !$request['ip'] or ($request['je'] =='TicketChatTable' and !$request['ip']) ){
                            $output['onReadyJs'] .= "$('#formMsTicketChat #msTicketChatBt').click();".$closeDiag."";
                        }else{
                            $output['onReadyJs'] .= "$('.child_pannel [p=\"TicketChat\"]').click();".$closeDiag."";
                        }
                    break;
                        ...
                        case 'sel':
                            ...
                            if(!$selectHtml){
                                $e = new TicketChatForm();
                                $e->TicketChat['request'] = $request;
                                $e->TicketChat['parentId'] = $request['ip'];
    ...
    setTimeout(function (){ $('#".$request['je']."').trigger('change'); }, 200);
                ...
                $f = new TicketChatForm();
                $f->TicketChat['request'] = $request;
                if(!empty($request['ip'])){$f->TicketChat['parentId'] = $request['ip'];}
                ...
                if(empty($error['error']) and @$_SESSION['memoire']['onglet']['TicketChat']['ixmemautoc']){
                    $autoc_temp = @$_SESSION['memoire']['onglet']['TicketChat']['ixmemautoc'];
                    $autoc_temp['i'] = $data['i'];
                    $autoc_temp['pk'] = 'IdTicketChat';
                    ...
                    unset($_SESSION['memoire']['onglet']['TicketChat']['ixmemautoc']);
                }
            ...
            case 'delete':
                ...
                $obj = TicketChatQuery::create()->findPk($request['i']);
                
                
                
                
                $_SESSION['CurrentRights'] = $_SESSION['CurrentParent'].'TicketChat';
                if(!$obj->delete()){
    ...
    if($request['a'] == 'edit'){
        ...
        #
        if($_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."TicketChat",'w')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."TicketChat",'a')
            or $_SESSION[_AUTH_VAR]->hasRights(@$request['pc']."TicketChat",'r')
    ...
    if($request['a'] == 'list'){
        if($_SESSION[_AUTH_VAR]->hasRights("TicketChat",'r')){
            init_list_base($request,$search,'TicketChat','','','','');
            $e = new TicketChatForm();
            $e->TicketChat['request'] = $request;
            if(!empty($request['ip'])){$e->TicketChat['parentId'] = $request['ip'];}
            
            $output = $e->getList(@$request['ui'],$request['pg'], json_decode(@$request['ip']), @$pmpoData, @$search);
            $siteTitle =$e->siteTitle;
            if($e->searchOrder){ $_SESSION['memoire']['search']['formOrderTicketChat']=$e->searchOrder;}
            if($e->searchMs){ $_SESSION['memoire']['search']['formMsTicketChat'] = http_build_query($e->searchMs);}
        }
    }
    
    function save_update_TicketChat($data, $request=array()){
        $e = new TicketChatForm();
        $e->TicketChat['request'] = $request;
        return $e->save_update_TicketChat($data);
    }
    function save_create_TicketChat($data, $request=array()){
        $e = new TicketChatForm();
        $e->TicketChat['request'] = $request;
        return $e->save_create_TicketChat($data);
    }
