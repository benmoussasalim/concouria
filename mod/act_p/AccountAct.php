...
switch($request['a']) {
    case 'refreshPayment':
        $_SESSION['payment_method'] = $request['method'];

        include _INSTALL_PATH.'mod/page/inc/AbonnementPrice.php';
        echo $return['AbonnementPrice'];
        die();
    break;
    ...
    case 'createAccount':
		...
		if(filter_var($data['email'], FILTER_VALIDATE_EMAIL)) {
			$accountCheck = AccountQuery::create()->filterByEmail($data['email'])->findOne();
			if(count($accountCheck) > 0) { $return['message'] = _("Ce courriel est déjà utilisé."); }
			else {
                $authyCheck = AuthyQuery::create()->filterByEmail($data['email'])->findOne();
                if(count($authyCheck) >= 1) { $idAuthy = $authyCheck->getIdAuthy(); }
                else {
				...
				if($idAuthy) {
					...
					if($mail) {
						...
						sendHTMLemail($body,_FROM,_FROM,$title);
                    ...
                    $return['error'] = false;
                    $return['class'] = 'create-account-success';
                    $return['message'] = _("Compte créé. Vous recevrez une confirmation par courriel. Redirection en cours.");

                    $form = new AuthyForm();
                    $form->tryLog($authyForm['Username'],$authyForm['PasswdHash']);
				}
			}
       ...
       // fputcsv($output,$exportArray,',',NULL);
        ...
        if($clients){
            ...
            foreach($clients as $client){
                ...
                $usernameContest = $client->getUsernameContest();

                if(!$usernameContest) {
					$username = substr(stripe_accent(str_replace('-','',str_replace(" ",'-',$client->getFirstname()))),0,3);
					$username .= substr(stripe_accent(str_replace('-','',str_replace(" ",'-',$client->getLastname()))),0,3);
                    ...
                    $client->setUsernameContest($username);
                    $client->save();

                    $usernameContest = $username;
				}
                ...
                array(
                    ...
                    strtolower($client->getEmail()),//16 - P
                  ...
                  //  $client->getMetroCard(),//29 - AC
                    $client->getHersheyUsername() ? $client->getHersheyUsername() : $usernameContest,//32 - AF
                    $client->getHersheyPassword() ? $client->getHersheyPassword() : $client->getPasswordContest(),//33 - AG
                    $client->getCantonUsername() ? $client->getCantonUsername() : $usernameContest,//34 - AH
                    NULL,
                    NULL,
                   ...
                   // $client->getFacebookPassword() ? $client->getFacebookPassword() : $client->getFacebookPassword(),//34 - AH
                    $client->getPresseUsername() ? $client->getPresseUsername() : $usernameContest,//37 - AK
                    $client->getCinocheUsername() ? $client->getCinocheUsername() : $usernameContest,//38 - AL
                    NULL,
		...
		$to = "salimbenmoussa@bcepharma.com";
		//$to = "david@progexpert.com";
		...
		if(count($code) > 0) {
			...
			if($code->getUsed() == 'Actif') {
				...
				if($return['error'] == false) {
					...
					if($code->getMontant() != '' AND $code->getMontant() > 0) {
                        $_SESSION['Price'] = (_PRIX_ABONNEMENT/100) - $code->getMontant();
                        $return['amount'] = '-'.$code->getMontant().'$';
                    }
					else if($code->getPourcent() != '' AND $code->getPourcent() > 0) {
                        if($code->getPourcent() == 100) {
                            $_SESSION['Price'] = 0;
                            $return['free'] = 'free-subscription';
                            $_SESSION['Promo']['Free'] = $return['free'];
                        } else {
    ...
    case 'PlanPayment':
        $return = returnDefault(_("Erreur lors de la requête."),true,'error');
        ...
        if($account) {
            require_once('/opt/stripe-php-6.16.0/init.php');
            \Stripe\Stripe::setApiKey($_SESSION[_AUTH_VAR]->get('group') == 'Admin' ? 'sk_test_gUdNqyBWn0x948Y2l1L6OEBu' : _SK_LIVE);

            if(!$account->getStripeCustomer()) {
                $customer = \Stripe\Customer::create(array(
                    "email" => $account->getEmail(),
                    "description" => $account->getFirstname()." ".$account->getLastname(),
                    "source" => $request['token']
                  ));

                if($customer['id']) {
                    $account->setStripeCustomer($customer['id']);
                    $account->save();
                }
            } else {
                $customer['id'] = $account->getStripeCustomer();
            }

            $type = $_SESSION[_AUTH_VAR]->get('group') == 'Admin' ? 'price_1Gs5yfHmbv3QikU8Kcq5KhZ5' : _STRIPE_PLAN;

            $_SESSION['payment_method'] = 'monthly'; //BYPASS
            
            if($_SESSION['payment_method'] == 'year') {
                ...
                if($charge['id']) {
                    $return = returnDefault(_("Abonnement créé avec succès."),false,'success');
                }
                ...
                if($taxes) {
                    foreach($taxes as $taxe) {
                        $taxeTotal += $taxe->getPourcent();
                    }
                }

                $_SESSION['Price'] = number_format(_PRIX_ABONNEMENT_MOIS * 12,2,'.','');
                $_SESSION['FinalPrice'] = number_format($_SESSION['Price'] + $taxeTotal,2,'.','');

                $subscription = \Stripe\Subscription::create(array(
                    "customer" => $customer['id'],
                    "billing" => "charge_automatically",
                    //"cancel_at_period_end" => true,
                    "tax_percent" => str_replace(',','.',$taxeTotal),
                    "metadata" =>
                        array(
                ...
                if($subscription['id']) {
                    $account->setStripeSubscription($subscription['id']);
                    $account->save();
        ...
        if(!$_SESSION['sale']) {
            ...
            $saleData['Statut'] = 'Refusée';
            $saleData['Date'] = date('Y-m-d H:i:s');

            $saleSave = $sale->save_create_Sale($saleData);
            $saleSave->save();

            $primaryKey = $saleSave->getPrimaryKey();
            $_SESSION['sale'] = $primaryKey;
        } else { $primaryKey = $_SESSION['sale']; }

        if($return['error'] == false AND $primaryKey) {
            if($account->getDateExpire() AND strtotime($account->getDateExpire()) < strtotime()) { $dateAdd = date('Y-m-d',strtotime($account->getDateExpire().' +1 year')); } else { $dateAdd = date('Y-m-d', strtotime('+1 year')); }

            $account->setExportReady('Oui');
            $account->setDateExpire($dateAdd);
            $account->save();

            SaleQuery::create()->filterByIdSale($primaryKey)->update(array('Statut' => 0));

            $abonnement = new AbonnementForm();
            ...
            $abonnementData['StripeResponse'] = $subscription['id'].$charge['id'];

            if($_SESSION['payment_method'] == 'year') {
                $abonnementData['Type'] = 'Annuel';
                $abonnementData['AbonnementPrice'] = (_PRIX_ABONNEMENT/100);
            } else {
                $abonnementData['Type'] = 'Mensuel';
                $abonnementData['AbonnementPrice'] = _PRIX_ABONNEMENT_MOIS * 12;
            }

            $abonnementSave = $abonnement->save_create_Abonnement($abonnementData);
            $abonnementSave->save();

            $abonnementKey = $abonnementSave->getPrimaryKey();

            if($abonnementKey) {
                $taxes = TaxeQuery::create()->useGrpTaxeQuery()->filterByIdGroupTaxeSup($account->getProvince()->getIdGrpTaxe())->endUse()->find();
                if($taxes) {
                    foreach($taxes as $taxe) {
                        $taxeSave = new SaleTaxeForm();
                        ...
                        $taxeSaveFinal = $taxeSave->save_create_SaleTaxe($dataTaxe);
                        $taxeSaveFinal->save();
                    }
                }
                ...
                if($mail) {
                    ...
                    $body = str_replace('%%lien_com%%',_SITE_URL_HTTP.'invoice?cmd='.md5($abonnementKey.'CONCOURIA'),$body);

                    sendHTMLemail($body,_FROM,($_SESSION[_AUTH_VAR]->get('group') == 'Admin' ? 'david@progexpert.com' : _SALE_EMAIL),$title,_FROM);
    ...
    case 'CancelPlan':
        ...
        if($account) {
            require_once('/opt/stripe-php-6.16.0/init.php');
            \Stripe\Stripe::setApiKey(_SK_LIVE);

            $sub = \Stripe\Subscription::retrieve($account->getStripeSubscription());
            $sub->cancel();
        }
    break;
	...
	case 'saveAccount':
        ...
        $accountSave['Couple'] = $form['couple'];

        if($form['couple_address'] != '' AND $form['couple'] == 'Oui') {
            $accountSave['Couple'] = 'Non';

            $couple = AccountQuery::create()->filterByEmail($form['couple_address'])->findOne();
            if(count($couple) >= 1) {
                $accountSave['Couple'] = 'Oui';

                $couple->setCouple('Oui');
                $couple->save();
            } else {
                AccountQuery::create()->filterByIdAccount($form['id-account'])->update(array('Couple' => 0));

                $return[2]['error'] = true;
                $return[2]['message'] = _("Adresse courriel associé au membre introuvable.");
                $return[2]['class'] = 'couple-email';
            }
        }
    ...
    case 'bypassPayment':
        ...
        if($account) { include _INSTALL_PATH.'mod/page/inc/ConfirmSale.php'; }
